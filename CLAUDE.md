...
- **License**: MIT
- **agent-ready-projects** (this repo): **v1.36.1 released; unreleased work on `master`** (proposed v1.37.0 — ***an instrument that agrees with itself is not evidence*** — MINOR. ⚠️ **`review-changes` Step 1.5 was a shell syntax error for eight releases and every check here was blind to it**: rule 6 compares two copies carrying the same apostrophe, so they agreed perfectly; the ratchet counts bytes; and every internal run transcribed the awk into a heredoc rather than copying the shipped block, which is why it worked for the maintainer and for no adopter. New **lint rule 11** parses every fenced block an adopter might copy — 26 measured, 2 failed — with a **declared** exemption, because the draft that guessed exempted `curate`'s extractor, the highest-traffic executable here. Rule 10 was blind the same way, to a runner named `mablate`. ⚠️ **Three changes were refuted by review before shipping and two of those refuted the previous round's fix**: a false measurement (two documents compared as one), a no-op remedy (it instructed the author to do what the finding's own precondition guarantees they had done), and a table split that made an open bug reachable *by following the instruction*. All authored here, none found by the author. ⚠️ **The `<!-- quoted -->` marker was built, reviewed five rounds and WITHDRAWN** — #76's gate is unmet, this repo holds zero instances of the class, and shipping a convention on one estate's evidence is what #16 was closed for. ⚠️ **The dead-reference extractor had SIX false-positive classes, three of them unfiled, and fixing it took two review rounds that found eight defects and eight more** — four of the second round's created by the first round's fixes. The sharpest was not a code defect: a measurement in the changelog was refuted because its *instrument* was wrong — brace members were checked with a bare `(root/member).is_file()`, which is #51's own false positive, so the claim came out backwards and in the direction that flattered the change (#121). **A negative measured with a mismatched instrument reads exactly like a real absence.** **Both issues worked this session turned out to be something other than what the ticket said**, which is why the remaining queue was triaged before being touched: 7 of 8 reproduce exactly as filed, #107's premise is refuted.)
...
bash tests/fixtures/private-names/run.sh            # sensitivity of lint rule 12
