...
- **License**: MIT
- **agent-ready-projects** (this repo): **v1.38.0 is the highest release (2026-09-06), tagged and pushed.** Proposed next bump **MINOR**. Session narrative lives in `memory/MEMORY.md` Current State; per-release detail in `CHANGELOG.md`. **The through-line, one line each — every one measured here, most of them twice:**
  - *An instrument that agrees with itself is not evidence.* Two copies carrying the same defect agree perfectly.
  - *A negative measured with a mismatched instrument reads exactly like a real absence* — and a **working-tree** grep cannot answer a question about **what ships** (`git grep <pattern> <ref>` can).
  - *A check that cannot run in the environment that runs it is not a gate* — a fixture without `set -e` certified an ordering broken only under `set -eo pipefail`.
  - *A fixture seeded by the author of the change can assert the hole is correct.*
  - *Half of classified review findings were defects the previous round's own fixes created*, so a round cap alone ships them — but that 52% belongs to four rounds, not to the ledger (8%).
  - *A fork inherits the author's blind spots*, and breadth buys the defects nobody predicted: on a four-lens round every lens found the top blocker while two each found one no other lens did.
  - *A number copied to a second surface stops being checkable against its source* — a doc lens verified a wrong statistic against the page that restated it.
  - *A budget whose escape hatch is always available and always taken is a log, not a gate* (H-022).
