...
- **License**: MIT
- **agent-ready-projects** (this repo): v1.35.0 (**`update-drift` Step 3 was comparing the wrong file, and this repo shipped that guidance in v1.32.0 on an adopter's unchecked premise.** Measured: `scripts/install-global-skills.sh` is a plain `cp` with **no transform** — the SAVE-AS→frontmatter conversion is maintainer-side, between `templates/` and `.claude/skills/`. So an install is byte-identical to one tracked file at exactly one tag: **0 differing lines against the reference install, 24 against the template**. Exact zero, not a minimum; #99's provenance stamp is closed as not needed. MINOR. ⚠️ **THE THING TO CARRY: three times in one day a peer's mechanism was shipped or filed here without being run** — `--depth 1`, `--filter=blob:none`, and the installer transform. All three arrived written as measurements, none was one, each cost under a minute to check, and *state the check before the claim* did not fire on any of them **in the session that wrote it down**.)
