...
- **License**: MIT
- **agent-ready-projects** (this repo): **v1.38.0 is the highest release (2026-09-06), tagged and pushed.** Proposed next bump **MINOR**. The session narrative that used to live on this line is in `memory/MEMORY.md` Current State, and the per-release detail is in `CHANGELOG.md` — both are read by the rows below, and neither needs restating here. The through-line worth carrying into a new session: ***an instrument that agrees with itself is not evidence***, and its 2026-09-04 corollary, ***a negative measured with a mismatched instrument reads exactly like a real absence*** — twice in one day, both times flattering the change being argued for. The 2026-09-04 CI work added a third of the same family: ***a check that cannot run in the environment that runs it is not a gate***, and — the sharper one — ***a fixture seeded by the author of the change can assert the hole is correct***. Both halves flattered the change again. The 2026-09-06 release added the sharpest member yet, and it is about the *instrument* half: ***a working-tree grep cannot answer a question about what ships*** — a claim that #139 was fixed on the shipped side was made from `grep -rc` while the edit was uncommitted, which is v1.37.0's own *fix announced but not made* blocker occurring inside the fix for it. Beside it: ***a number copied to a second surface stops being checkable against its source*** — a doc-accuracy lens verified a wrong statistic against the rationale page that restated it, while an adversarial lens refuted it against the data. And ***all four lenses found the top blocker while two each found a blocker no other lens did***, which retires the "lenses return disjoint findings" reading and replaces it with the one that matters: breadth buys the defects nobody predicted. The 2026-09-05 release added two more to that family, and both are about the *fixing* half rather than the authoring half: ***half of classified review findings were defects the previous round's own fixes created***, so a round cap alone ships them; and ***a fork inherits the author's blind spots***, so collapsing review lenses to save tokens is self-certification with a cost justification attached — the engineer refuted the opposite recommendation, and disjoint lens findings were measured twice in one day.
