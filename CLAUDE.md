...
- **License**: MIT
- **agent-ready-projects** (this repo): v1.34.2 (**v1.33.0 stopped reporting a false *dead* and started withholding a true one.** An adopter measured it against the one reference their repo keeps unfixed on purpose as a control: sibling on disk, file provably absent there, reported *not checkable*. MINOR. Two to carry: (1) **#93's sentence points both ways** — the earlier bug claimed a verdict it had not earned, this one withheld one it had, and v1.29.0 had already settled the shape (a third state for the genuinely undecided, not a dropped rung). The gate here is materially different from rung 4's because **the fragment qualifies itself** rather than needing a repo name parsed out of prose; the residual environment-dependence is *dead* vs *undecided*, never a false *dead*. (2) **A version stamp is a number, not an adjective** — an adopter's word *current* was false within hours because two releases tagged the same evening, and **our own release cadence is what falsifies it**, which they cannot see coming. ⚠️ **Both of the dead-reference extractor's false-positive classes AND this false-negative were found by adopters running it, never by anything here.** ⚠️ **v1.34.1 corrects a claim v1.34.0 made one hour earlier**: the sibling rung's exposure is *sparse checkout only* — measured, `--depth 1` truncates history and `--filter=blob:none` fetches blobs at checkout, so both leave every file present. I wrote *"shallow or partial"* by accepting a peer's plausible mechanism **without running it**, in the session that recorded *state the check before the claim*. The case is now seeded as a known, unfixed exposure. ⚠️ **v1.34.2: the correction had an unmeasured row of its own.** `--filter=blob:none` was claimed as measured by BOTH sessions and by neither — a local `file://` remote answers *filtering not recognized by server, ignoring* while still writing `promisor=true` and `partialclonefilter=blob:none` into the config, so the clone looks partial by every flag and has zero missing blobs; my run hid the warning behind `2>/dev/null`. **Third instance in one thread of a claim written as measured and not measured, and the third is mine, inside the correction of the first two.**)
