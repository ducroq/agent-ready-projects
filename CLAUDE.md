...
│       ├── dead-reference/      <- Seeded classes for curate Step 0.1's extractor: 40 rows,
      ...
      │                          review-changes Step 1.5. Counts are COMMANDS: the
...
| `tests/fixtures/step15-tables/` | Seeded input for `review-changes` Step 1.5 (#50, #52, #103, #144). Its emphasis rule took three drafts, each refuted by running it over **this repo** (28 hits, then 15, then 1) rather than over the fixture — every draft passed the fixture. Read that history before loosening the bold-nesting test |
