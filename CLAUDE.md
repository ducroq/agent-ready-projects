...
- **License**: MIT
- **agent-ready-projects** (this repo): v1.31.0 (**eleven issues closed in one day, 21 open → 10 → 0.** MINOR. Most were one failure — *a check that reports nothing when broken, indistinguishable from reporting nothing because all is well*. Three to carry: (1) **every new checker was refuted by running it over THIS repo, never by its own fixture** — the #50 emphasis rule reported 28 lines, then 15, before adjacency made it 1, and the #51 extractor reported four of its own false positives on first run; (2) **two silent failures in the #46 read-surface gate both read LOW** — a spaced path word-split away half the total, and xargs batching made `tail -1` report 105,600 of 600,000; (3) **four separately-filed adopter issues were one rule seen four ways** and shipped as one, per v1.30.0's lesson that a rule restated in seven passages gets corrected in one and left wrong in six. Four review rounds ran; the fork-context ones cost a fifth of a cold one and found more.)
