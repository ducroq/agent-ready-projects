...
- **License**: MIT
- **agent-ready-projects** (this repo): v1.30.0 (marking a reference the LOCAL tree answers made the run undecided — Step 4's marked arm ran rungs 1, 3, 4 and **skipped 1b and 2**, so v1.29.0's new exit-2 fall-through turned that gap into #93's own defect a **third** time. MINOR, because rung 1b now *adjudicates*. **Three review rounds, 5/6/7, all finding real defects; the suite was green at the start of each.** Three to carry: (1) **the fix added a new excusing arm and omitted the one guard every other excusing arm has** — a silent drop from all six counted sections passed the whole suite, because X17/X18 assert `VERDICT: CLEAN` and a silent drop satisfies that; (2) **the consolidation table shipped a false row** — *resolves nowhere, neighbour reachable* said `Findings` when it is the correctly-marked placeholder, excused at exit 0; nothing checks a table against the thing it describes, and the oracle agreed with the prose, not the table; (3) **the unhedged absolute the release is named for was re-created verbatim inside the fix for it**, one screen above its own hedge. Round 7 was scoped to prose only, cost half a full round, and found the worst of the three.)
