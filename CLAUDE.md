...
| Before committing structural changes (CLAUDE.md, `memory/`, `templates/`) | Run `bash tests/lint/run.sh` — deterministic structural rules, count printed by the run; **`tests/lint/README.md` is the catalog and states what each rule cannot see.** **CI runs it and `bash tests/run-fixtures.sh` on every push and PR** (`.github/workflows/checks.yml`, #115), so a green tick is available to be mistaken for a review: it is not one, `/review-changes` still is. Five rules SKIP in CI and cannot do otherwise — rules 2 and 15 lack `memory/`, rule 12 a name list, rules 18 and 19 tags — so **the local run is the stronger one**, and the skips print to the run summary rather than folding into the pass. Then run `/review-changes` for diff-driven LLM review: tiers from `.claude/review-profile.md` (v1.40.0), size gate from the skill; with no profile it stops rather than defaulting to LOW. |
...
bash tests/lint/run.sh                              # structural rules; 2, 12, 15, 18, 19 SKIP where their input is absent (CI)
