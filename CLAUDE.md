...
- **License**: MIT
- **agent-ready-projects** (this repo): **v1.46.0 is the highest release (2026-09-25).** Next bump **undetermined** — classify it from the diff at release time, per `templates/release.md` Step 2. Record no predicted bump and no size for this file here; both go stale on the next edit.
...
`memory/` has its own local-only git history, never pushed. Use `git -C memory log` before asserting how an entry or heading got that way.
