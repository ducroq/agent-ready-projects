...
**Proposed bump when it does go out: MINOR** — new behaviour on every adopter-facing surface this repo has. Checkable rather than counted, because a count of *four* stood here while more had already changed and the number is the kind of thing that goes stale silently: `git diff --name-only v1.36.1..HEAD -- templates/ README.md adopt.md docs/GUIDE.md`. New files too — `docs/seeded-defects-and-ablations.md` and five `docs/rationale/` pages.

**MINOR** — a shipped block that had not parsed for eight releases, two rung-4 defects in `audit-context`, six false-positive classes in `curate`'s dead-reference extractor, and two lint rules that could not see their own subject. Closes #103, #105, #112, #102, #104, #106, #115. **Seven** issues were filed from the same work rather than folded into it: #116–#122. #121 and #122 came out of the `curate` work below — one is a cost this release takes knowingly and measures, the other a divergence between two steps that this release makes visible.

**The through-line, since it decides how to read the rest.** Every fix here was found by running the thing, not by reading it — and four of the five were invisible to the checks that exist precisely to catch them. The fourth is the newest: `curate`'s dead-reference fixture was green on all six of the extractor's false-positive classes, because it had no case for any of them, and the one check that compares the two skill copies compared two files carrying the same defect. `review-changes` Step 1.5 was a shell syntax error that rule 6 could not see, because both copies carried the same defect and agreed perfectly. Lint rule 10 could not see an ablation runner whose name merely ends in `ablate`. `audit-context` printed a remedy that does not work, and the step's own gloss forty lines away was correct the whole time. **An instrument that agrees with itself is not evidence.**
