...
## Unreleased (proposed MINOR — v1.39.0)
...
**A UTF-8 BOM defeated the skip outright (#151 part 2).** `NR == 1` cannot match `\xef\xbb\xbf---`, so the exact false positive the skip exists to remove came back. Stripped now, in octal rather than `\x`, which is a gawk extension. ⚠️ **The estate holds zero BOM files**, so a real-tree run cannot tell this fix from a disabled one; **N9** and ablation **A9** are the whole evidence.
...
**`CLAUDE.md`'s fixture counts become commands, having been wrong before this change touched them.** It claimed *"40 rows, 16 ablations"* for `dead-reference` while the file at HEAD held **35** and **15** — #93's class, fourth instance, found by measuring rather than by reading.
