...
**Budget: net DOWN 43 bytes across the release** (241,524 → 241,481), which is five payments and one raise, not a single act. ⚠️ **This figure has been rewritten four times, once per review round** — every round changed a template, and the sentence measures the templates. It is re-derived here after the last edit that can move it, never carried forward: `grep '^# BUDGET' tests/lint/size-baseline.tsv` against the same line at `origin/master`. **Payment 1:** `templates/update-drift.md`'s `SAVE AS` clause cost +192, covered by deleting two rationale paragraphs from that same comment — *why user-global* and *why the skill earns a slot* — which `docs/GUIDE.md` now states in full and which the comment already pointed at; net **−79**. **Payment 2 and the raise:** the `templates/review-changes.md` correction below cost +285; 221 of it was paid by deleting a paragraph that restated the `description:` field three lines beneath it *and* the body's own opening sentence — and was **stale on top of that**: it said the skill reviews changes *"against the previous commit"*, which Step 1 has not done since it began resolving `$BASE` to the default branch — and the residual **+49 was raised deliberately** with that arithmetic as the reason. ⚠️ **That raise note's two summands were both 15 bytes high** — while their *difference* was right and pinned by the row; corrected in the note to +270 and 221. ⚠️ **A cause was then offered for the 15 and withdrawn on the next round** — the prefix it blamed is 14 characters, and the arithmetic gives 284. **Numbers about numbers have now been wrong three times in this one ledger.** **Payments 3 and 4:** round 3's two header fixes cost +245 and were paid entirely, 20 of it from a paragraph in `templates/test-verify-memory.md` that was not merely redundant but **wrong** (it cited *curate Step 0.5*; the body beneath it says *Step 0, sub-step 5*). ⚠️ **A draft of this paragraph said "RATCHETED DOWN 79, paid rather than raised" and was true when written** — the later fixes arrived a round and two rounds afterwards and made it false, twice. **H-022 gets a genuine mixed datum**: the hatch was declined once and taken once in one change, and the taking was for 17% of what the fix cost. **A second copy of an argument is a second copy to keep true** — which is what both payments were.
