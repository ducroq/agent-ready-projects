...
**Size.** `templates/audit-context.md` grows 43352 → 44784 bytes (+1432) and the rule 8 baseline is updated deliberately. It buys a corrected rung order, the decidable/mislabelled distinction, the scope on two refuted absolutes, and a hedge on a third: *"rung 4 declines an angle-bracket segment wherever the audit runs"* is refuted by a neighbour holding a literal `<…>` filename, which suffix-matches (rare — illegal on NTFS — and real).

### Round 6, and what it says about the shape of this document

Round 6 ran on round 5's fix and found nine more defects. Three were mechanical and are fixed here:
...
⚠️ **The other five were all one shape: a rule restated in five places, edited in one.** A stale rung list ("at rung 1 or rung 4") four paragraphs above the new text; "a path a local rung answers stays in the skip section", true for rungs 2 and 3 and false for 1 and 1b; a correction written in the present tense about behaviour the same commit removed; "every rung-2, rung-3 and rung-4 resolution" in the reporting section, widened by the fix and not updated. **Step 4 stated the rung order and its consequences in five separate passages**, so every release corrected one and left four — which is a fair description of rounds 1 through 6 as a whole, not just this one.

**So this release consolidates rather than adds.** One table states, once, what a marked path does at each rung: which rungs *adjudicate*, which only *decide*, and which counted section each outcome lands in. The other passages point at it instead of restating it. That is why the net growth is +1432 bytes and not more — the table costs roughly what the removed restatements did, and the duplication is what actually went away.
