...
Rule 2's silence is the trap rules 6 and 12 are both built around — an absent population and a clean one producing byte-identical output — sitting two lines above them, in the rule that runs first. Rule 2 now reports a **SKIP** and counts it; rule 1 exempts a path only when it is absent **and** gitignored **and** under one of the two documented maintainer dirs, and **prints how many it exempted**. CI's own run: `27 file and 17 directory reference(s) checked; 4 + 1 exempt` and `2 rule(s) SKIPPED — a skipped rule is not a passed rule`, with the skips surfaced to the run summary rather than folded into the pass.
...
**No path filters, deliberately** — a stated deviation from the issue's own plan. Scoping each suite to the paths it covers saves about two minutes of a **2m15s** local run (37s on GitHub's runners, where the whole set is cheaper than the argument for trimming it) and buys back exactly the selection mechanism the workflow exists to remove. `tests/run-fixtures.sh` **enumerates** `tests/fixtures/` instead of listing it, so a suite added tomorrow runs tonight, and it refuses three silences: an empty population exits 2, a fixture dir with no runner FAILS unless declared a non-gate, and one red suite never stops the other twelve. `review-bench` is that declared non-gate — it scores a review configuration and has no pass/fail by design; wiring it in would turn a measurement into a threshold nobody chose.
