...
- **Lint rule 17 widened (#160, maintainer infrastructure).** A `#` welded to a word after `shopt`, `export`,
  ...
  passes all of them. Quote-aware, and quiet on `=#`, `$#`, `${#` and a `#` inside a word. Fixture T11, N4, T12, A5.
...
- **`curate` Step 0 sub-step 6 (#143).** Files the project file tells the agent to read every session are listed on
  their own line with their sizes, without claiming they are always read.
- **`gotcha-log.md` Promote phase (#182).** Word a promoted pattern without its first instance's domain, and check
...
curate 45.5k → 23k characters, review-changes 43.5k → 27k, paid on every run. Code blocks and step
numbers are unchanged; narratives and issue histories moved to `docs/rationale/`. One instruction
changed: curate names exit 127 (runner not found).
...
What ships, on four surfaces:

- **`docs/GUIDE.md`** — an install paragraph in the rhythm section, a **global** row in the "Where a skill lives" table, and a **Start of session, after a framework you pin ships a release** row in the Documentation Rhythm table. That last one closes the section's own argument: *"Why these five"* reasons from every skill attaching to a row, and the fifth skill had no row. `templates/project-file.md` — the file adopters copy — has carried that row since v1.28.0, so the guide was behind the template it ships.
- **`README.md`** — item 16 in "Growing from there", plus inventory bullets for `update-drift.md` **and** `review-profile.md`, which the same table had been missing since v1.40.0.
- **`adopt.md`** STEP 6 — `update-drift` added to the user-global install list, to the Before-You-Start pointers it tells you to write, and to the other-tools fallback.
- **`templates/update-drift.md`** — the `SAVE AS` comment gains the *do not copy this file verbatim, its frontmatter is inside this comment* clause that `curate` and `audit-context` have carried since **v1.15.0** — three releases before `update-drift` shipped, and never back-filled onto it. Its absence is not decorative: nothing on the adopter's path warned them, and the first draft of this very release wrote the wrong install instruction into `docs/GUIDE.md` because of it.

🔴 **The review found that first draft's install line was the defect it was documenting.** It said *install [`templates/update-drift.md`] as `~/.claude/skills/update-drift/SKILL.md`* — and that file's frontmatter lives inside its `SAVE AS` comment, so a literal copy **never registers, silently**. `tests/lint/run.sh` records it in the comment beginning *"Rule 3 checks that each template CARRIES installable frontmatter"* — an adopter repo found holding all three skills that way, none of them ever loaded, for months. (Anchored on the phrase, not a line number: that file is under active edit.) It also contradicted rule 1 of the section it linked to (*install global skills from the tracked copies in `.claude/skills/`*), 39 lines below. **The same defect was already shipped one paragraph over** for `review-changes` — since **v1.13.0**, through every release since (`git show v1.13.0:docs/GUIDE.md`); v1.40.0 changed that skill's *scope*, never its source. Fixed here too, because fixing a defect does not immunise you against it one file over.

**A Layer-4 log needs a history, and the framework never said so.** `templates/gotcha-log.md` prescribes the file, tells you *don't delete — it's history*, and is silent on whether that history exists. `git grep -rniE 'archiv|retire' HEAD -- templates/ docs/GUIDE.md README.md` returned **42** hits **before this change** and not one named a version-control disposition; a positive would have read *"track it"* or *"give it a local history"*. ⚠️ **Run against the working tree it now returns 44 and two of them do** — the two this change adds. The claim is about the state being fixed, so it is pinned to `HEAD`; a draft wrote it present-tense, which is the published-command-that-contradicts-you shape confessed three paragraphs above.

The questions a gotcha log is eventually asked are about **change**, not about its current text — *did a retire pass rewrite these headings on the way to closing them*, *when did this entry's status really move*, *was every entry present before a split present exactly once after*. A log with no history answers none of them, **ever**, and silently: it looks identical to one that can.

**One adopter and this repo hit it from opposite sides, ten minutes apart.** The adopter who filed #178 could measure their own archive-vs-live split because their log lives in a tracked `docs/`; this repo could not run the settling command on its own log at all — `git ls-files memory/` → **0**, `git log -- memory/gotcha-log.md` → **0 commits** (both still true from the repo root; a **local-only** history has since been created inside that directory — see `CLAUDE.md`) — which blocked #178's own verification and left **unrunnable** the arithmetic gate that would have settled #180's temporal half. ⚠️ A draft said the missing history *killed* that temporal half; it did not — a selection confound in the adopter's archive did, and this repo produced its own entry-date split (51% → 64%) with no history at all. **The gate is the thing that needs `git log`, and it is the thing that could not run.**

⛔ **And the obvious remedy is a trap, with a number.** Tracking an ignored log publishes whatever is in it, and **the check that would catch that has never read it**: a private-name denylist enforced over tracked files does not scan ignored ones, because ignored files are not in its population. Measured here before deciding: **11 of the 34 names rule 12 checks (the list holds 38; four are reported UNCHECKED as too short or generic) appear somewhere under its ignored `memory/` directory**, and lint rule 12 had never read a byte of it. So the guidance names the local-only form — `git init` inside the ignored directory, never a remote — which answers the same questions and publishes nothing. `docs/GUIDE.md`'s retirement section carries the reasoning; the template carries the rule.

**#174 — the `⊘` placeholder marker shipped in v1.41.0 was decorative, and the remedy is the reverse of the one filed.** The issue proposed teaching the checker the glyph. Measured instead, in this repo's real tree rather than a fixture — **this is the BEFORE state, 2026-09-13**, from `python3 tests/fixtures/reference-integrity/refcheck.py --sibling-root ~/repos . memory/gotcha-log.md` (condensed; the real output carries a third column). All four now bucket as `declared-placeholder`, and that log is gitignored, so no reader can reconstruct this:

```
tests/lint/count-commands.sh    UNRESOLVED           <- ⊘ only
tests/lint/opt-comment.sh       UNRESOLVED           <- ⊘ only
tests/lint/raise-ordinal.sh     UNRESOLVED           <- ⊘ only
tests/lint/save-as-header.sh    declared-placeholder <- <!-- placeholder -->
```

A `⊘`-marked path is reported **identically to an unmarked one**. So three of this repo's own `proposed` rows were standing false findings for a release, each one written by an author who believed the finding suppressed — **which is worse than no marker at all, because it stops the author looking.**

`<!-- placeholder -->` is already implemented, already in `SPEC.md`, already the fixture's best-covered marker (**14 seeded cases**), and **works inside a table cell** — verified against this repo's own Mechanized table, and now seeded (below) — which is the shape the Mechanized table needs. So `templates/review-changes.md` Step 3.1 now prescribes it and the glyph is withdrawn. ⚠️ **Converting the rows was not the fix — two review rounds found the instruction and the example still teaching the old form.** The gotcha log's own header still said *a proposed row's check path carries a leading ⊘*, so the next row written by following it re-created the class; and `templates/gotcha-log.md`'s shipped Mechanized example showed an em dash in the Check cell while its `live` example path carried no marker at all — **a standing false finding of the class being fixed, shipped in the template that defines the table.** Both corrected. That is a *smaller* adopter surface than implementing a second marker form, and #177 names the cost of the alternative: *a third form to get wrong*.

**`templates/audit-context.md` Step 4 names a marker syntax for the first time.** It said **mark them** and never said how — it deferred the rules to `SPEC.md` and to a checker that is an oracle, never installed. An adopter running the audit as prose therefore had no marker to apply and no way to clear a residue the step tells them to clear. It now gives the form, says it is span-scoped so it must sit on the line it excuses, and says it works in a table cell.

🔴 **The obvious simplification was proposed, measured, and refuted — record it so it is not re-proposed.** The Mechanized table already has a `Status` column saying `proposed`, which *means* the file does not exist yet; a markdown row is one line, so "skip unresolved paths in a row whose status is `proposed`" is exactly as line-local as the marker and needs no token, no placement rule, and no escaping. It looked like a deletion rather than an addition, which is the direction this repo prefers.

**It is a loosening, and the measurement is one row.** Counting extractable paths per `proposed` row in this repo's own log: **1, 3, 1, and 7**. The seven-path row names `templates/review-changes.md`, `docs/GUIDE.md`, `templates/README.md` and `scripts/install-global-skills.sh` — all real, all resolving — beside the one path that does not exist. The marker excuses exactly that one; a row-wide skip would silence **six real files**, so a later rename would be reported by nothing. That is precisely the neighbour-silencing `SPEC.md` bounds the marker against (*"the marker is span-scoped so it cannot silence a neighbour"*), arrived at from the opposite direction.

**So span-scoping is not incidental — it is the feature**, and the cost of it is the placement rule that took three drafts. A narrower version (skip only the *Check* cell's path) needs the checker to know which cell is which, which means parsing the table header: more machinery than the marker, not less. ⚠️ **The prior art agrees with the shape and not with the token.** `# noqa`, `// eslint-disable-next-line`, `# type: ignore`, `# shellcheck disable` are all inline suppressions with unused-suppression detection — which `STALE PLACEHOLDER MARKER` and `COVERS NO PATH` independently reinvent. But every one of them is a **line comment in the host language**, which is why none of them can terminate the construct it is written inside. Ours can, and did.

⚠️ **Three seeded cases, and the first draft of the guard did not work.** `T46` asserts a `⊘`-marked path is still *reported*; `N42` and `T47` are the table-cell pair the prescription needs — the marker in the path's own cell is skipped, a marker parked at the **end of the row** does not excuse the path in an earlier cell. Before this the fixture carried 23 markers and **zero on a table row**, so the placement both skills now prescribe was unseeded.

🔴 **`T46` was seeded in the wrong position and a review lens refuted the claim made for it.** The glyph sat *before* the path; the claim was that anyone implementing it without changing the prescription gets a red suite. **Measured false**: teaching `PLACEHOLDER_RE` the glyph — the obvious one-character implementation — left T46 **green**, because the shipped marker is backward-scoping, and an unrelated row went red with a *wrong diagnosis* instead. The glyph now sits where the working marker sits, and that same ablation turns T46 red. ⚠️ **Two further drafts were wrong about what the misplaced marker does**: it does not report the accused path stale — the path resolves at rung 2, where a marked path skips the local rung, so the marker is **silently consumed and nothing reports the misplacement at all.** Worse than the drafted story, and the seeded case now asserts the measured one.

⚠️ **The mutants are run by hand, not committed** — nothing in this harness can reach a main-fixture case row, which the fixture already says of itself elsewhere. Saying so is the point: a described ablation presented as a committed one is the thing that file exists to refuse.

🔴 **The install-source defect was in SIX skill templates, and it took three review rounds to stop finding more (#184).** Round 1 found it in the paragraph being written. Round 2 found the third instance *in the same list the first two were fixed in*. Round 3 found the fourth — `templates/release.md`'s own header, which by then **contradicted a line round 2 had just written into `docs/GUIDE.md`** — plus `templates/test-verify-memory.md`. **Each round fixed the instances it was shown and none of them asked how many there were.** The question that would have ended it on round 1 is one command: `for f in templates/*.md; do sed -n '3p' "$f"; done | grep 'SAVE AS'`, then count how many carry the do-not-copy clause — it was **four of six**, and is now six of six. `templates/release.md` carries its frontmatter inside a `SAVE AS` comment exactly like the other two, and `docs/GUIDE.md`'s **Releases.** paragraph was still sourcing the install from it. All five `docs/GUIDE.md` install paragraphs now name a clone's ready-made `.claude/skills/<name>/SKILL.md`, and all six skill templates carry the clause in their own headers. ⚠️ `templates/test-verify-memory.md` gets only half of it — it has no reference install to prefer — and whether it should have one is an **open decision**, not a settled one: `tests/lint/skill-sync.sh` and `CHANGELOG.md` v1.20.0 both say in terms that it is *"a separate question"*. A draft of this line wrote *by design*, settling in an adopter-facing template what the repo's own record twice declines to settle. So the half of the clause about the defect is all that applies. ⚠️ **The `review-changes` instance is not v1.41.0's**: `git show v1.13.0:docs/GUIDE.md` already sources it from the template, so the defect is **thirty-seven releases old** (`awk '/^## v/{n++} /^## v1\.13\.0/{print n-1; exit}' CHANGELOG.md`) and v1.40.0 changed only that skill's scope. ⚠️ **A draft of this sentence said twenty-eight** — `update-drift`'s number, copied onto a different subject, in the entry whose subject is numbers that stop matching what they count.

🔴 **`templates/review-changes.md`'s own `SAVE AS` line still said project-local** — no `~/`, no USER-GLOBAL marker, no do-not-copy clause — while `docs/GUIDE.md`, `templates/README.md`, `adopt.md`, `CLAUDE.md` and `scripts/install-global-skills.sh` have all made it user-global since v1.40.0, and the guide says a project-local copy is now *inert*. An adopter obeying the file's own header installed a shadowed skill that never loads. **No lint rule can see this**: rule 3 checks only that `name:`/`description:` are present, and rule 6 excludes the `SAVE AS` block from its comparison by construction, so both copies agree while contradicting five other surfaces. Now corrected and marked INERT-if-local.

**`adopt.md` shed three more stale enumerations, all of the same shape as #173 itself.** Its raw-URL list named two of the four global skills; its *do not copy* warning named two of the four templates; and the pointer sentence told adopters to add rows that `templates/project-file.md` already ships. ⚠️ **Round 2 carved out one of those rows and left the other three**, so the sentence then warned against the duplication it was still prescribing — the same fix-one-instance shape as the headers above, in the same batch. STEP 6 now says to **check** that table and add nothing: the template ships a row for all five skills, each with its paste-the-template fallback. Each stale enumeration is now parameterised (`<name>`) or deferred to the file that owns it. ⚠️ **Lint rule 7 is blind to the duplicate-row class**: it derives its canonical trigger only from the row targeting `memory/MEMORY.md`.

**Scope: global, and the guide's own test is what says so** — *count the references to paths that exist in one repo only*. The command is `` `grep -o '`[^`]*`' templates/update-drift.md | grep '/' | sort -u` ``; it returns eleven tokens, ⚠️ **and the first draft of this sentence shipped it with the backticks backslash-escaped, where it returns 0** — inert escapes inside a code span, and GNU grep reading `` \` `` as a start-of-buffer anchor. A published check that returns a negative for instrument reasons, in the paragraph arguing from that check; and a positive would be one naming a directory in the **adopter's** tree and nowhere else. There is none — every token is a convention path, a `<framework>`/`<name>` parameter, or a path in the *framework's* tree, which is this skill's subject rather than its host. ⚠️ **Two absolutes in the first draft did not survive**: the skill does name `~/repos/<framework>/`, offered *if one exists* rather than required, and *"a per-repo variant would have nothing to vary"* is refuted by the skill's own Step 0, where the directories to sweep are chosen per repo — at run time rather than baked in, which is what actually licenses one global copy.

**Bump: MINOR, on the v1.28.0 / #68 precedent rather than on re-derivation.** `templates/release.md` Step 2 rule 2 asks whether there is a *new artifact* to adopt, and there is not — `update-drift` shipped 28 releases ago and every file here already existed. v1.28.0 is the near-exact case: a shipped surface (`templates/project-file.md`) that named none of the five skills, fixed by naming them, released MINOR. ⚠️ **A review lens cited this precedent as v1.29.0 and it is v1.28.0** — `git describe --tags --contains fe746d3` → `v1.28.0~2`. ⚠️ **That entry also says the guide *"prescribes an eight-row documentation rhythm"*; the row added here makes it nine.** True when written and left standing as history — but a future grep for the phrase will find a ninth row, so it is named here rather than silently outrun. **Adopter action: none forced.** `scripts/install-global-skills.sh` has installed `update-drift` since v1.18.0, so anyone who ran the script already has it; this release is what tells the adopters who read the docs instead.

**Budget: net UP 1,707 bytes across the release** (241,524 → 243,231). **Six raises against four payments.** The payments were real duplication — paragraphs restating a `description:` field three lines beneath them, one of them also stale, and a sentence standing twice in one step. ⚠️ **But every raise after the first was a CORRECTION found by review, not new content**: a positionally ambiguous rule that permitted a worse failure than the bug it fixed, a template pointer dead in every adopter repo, a shipped example contradicting the rule being added. That is a different pattern from the one H-022 predicts — the hypothesis is that an always-available hatch is always taken *for growth*; here it was taken to pay for getting things wrong. **Both readings end with the budget up, and the entry should not pretend the second is better.** Re-derive after the last edit that can move it, never carry it forward: `grep '^# BUDGET' tests/lint/size-baseline.tsv` against the same line at `origin/master`. ⚠️ **This figure has now been rewritten six times, once per round** — every round changed a template, and the sentence measures the templates. **That is the strongest single argument in this entry for mechanizing the number instead of writing it**, and it is why the sentence now names the command rather than carrying the digit forward.

**`CLAUDE.md` grew 314 characters** (36,418 → 36,732; `git show origin/master:CLAUDE.md | wc -m` against `wc -m CLAUDE.md`), putting the auto-loaded set at 37,856 of its 40,000 hard cap. Well inside the cap, and still over the 35,000 **soft** flag `templates/audit-context.md` Step 1 owns — which the 3,997-character reclamation pass earlier in **this same unreleased batch** did not clear either. ⚠️ A draft credited that pass to v1.41.0; `git show v1.41.0:CLAUDE.md | wc -m` and v1.39.0 and v1.40.0 all return **38,600**, so `CLAUDE.md` did not move in any of them. Recorded because a change that grows an already-flagged file should say so.

Maintainer-local, same issue: `CLAUDE.md`'s two prose inventories of `.claude/`'s tracked children said `skills/` only, while the tree also tracks `review-profile.md`. Both now name both, and the *What is intentionally not shipped* row carries the command — that table's stated job is being honest about what a `git clone` contains, so an omission there is against its purpose. ⚠️ **Lint rule 1 caught the first draft of that fix**: it wrote the children as `` `skills/` `` and `` `review-profile.md` ``, which resolve from no root.

## v1.41.0 (2026-09-12)

**`review-changes` gains a promotion path from a review finding to a deterministic check (#127).** Filed by an adopter alongside #126, and named by this repo's own H-020 as *"the whole answer"* to review cost. The framework already has a promotion path for gotchas — a recurrence becomes a written pattern, with an Occurrences column to catch a promotion that did not take. **There was no equivalent for review findings**, so every session paid full lens price to re-discover shapes that were already known and often trivially scriptable.

What ships:

- **Step 3.1 — Mechanization triage.** Runs inside synthesis, on findings already in context. Spawns no subagent. Per finding: could a deterministic check have found this? Yes → name what it greps, parses or runs and the file it lands in. No → say why in four words. **A blank is not an answer**; it is indistinguishable from a finding nobody triaged.
- **A destination.** `templates/gotcha-log.md` gains a **Mechanized** table. Separate from Promoted rather than folded into it, because the destinations differ in kind: a gotcha becomes prose an agent reads, a review finding becomes a check that runs.
- **A `### Mechanizable` section in the Step 4 report**, with the same never-omit rule the Unclassified section carries.
- **`curate` reads the new table** (Step 2), so a `proposed` row that nobody builds and a `live` check that stopped firing both surface. Added after review found the table had no reader at all — a write-only destination, which is the "a config that is never loaded looks identical to one that is correct" failure this repo has logged before.

**`proposed` vs `live` is the load-bearing distinction**, and it is the issue's own caution. Naming a check is not writing one. A row goes `live` only once a seeded positive has made the check go red — never on the author's read of it — because *a check that has never caught anything is indistinguishable from one that does not work*.

🔴 **`curate`'s Promoted-table extractor was broken by this change and is fixed in it.** `awk '/^#+ Promoted/,0'` is a range to end-of-file, so appending any section after Promoted pulls it into the same read: **11 rows returned where 7 exist**, four of them from a five-column table with different semantics. The replacement is bounded at the next **same-or-shallower** heading — level-aware, not `/^#+ /`, because entries nest as `###` under `## Promoted` in real logs and exiting at any heading returns **zero** rows there, which is the silent-empty direction. Both failures were measured before and after; the naive fix was written, tested, and discarded for the second one.

⚠️ **The yield of this step is UNMEASURED, and the table it ships with was seeded retrospectively.** The rows in this repo's own Mechanized table come from triaging findings already in the record, not from a round executing Step 3.1. **The first live run is the measurement.** #127 is explicit that it is *"Not measured: whether mechanizing shapes actually reduces future round cost"*; nothing here changes that.

🔴 **The first proposed check was REFUTED by the review that reviewed it, and that is the most useful thing in this release.** The retrospective triage named one class — *a case count stated as a literal in a fixture header goes stale when a case is added* — and proposed the check *"must express the count as a command, not a digit."* The adversarial lens tested that against the live instance and it fails: `tests/fixtures/reference-integrity/README.md` states **27 T-cases and 28 N-cases** where the run file holds **38 and 35**, and it **already carries the re-deriving command immediately beside the stale digits**, plus a sentence warning that any number in prose there is dated. A check spelled "must carry a command" passes that file while its numbers are wrong. **The predicate is therefore not "is a command present" but "does the stated number equal what the command returns"** — run it and compare. The row is re-scoped accordingly and stays `proposed`. ⚠️ **The lens also refuted the row's population**: of six enumerated sightings, at most one occurred in a fixture header or README (#97 double-counts sighting 1; #162 is the review ledger, outside the class). Corrected to **one live instance in the stated population**, with the rest recorded as the wider class they belong to.

⚠️ **Adopter action.** Re-install `review-changes` (global since v1.40.0, so one install covers every repo) and `curate` (global; it carries the extractor fix, which matters to any adopter whose gotcha log has *any* section after Promoted). **Do not re-copy `templates/gotcha-log.md`** — it is an append-only journal holding your real entries, and re-copying destroys them. **Append** the `## Mechanized` section to the log you already have.

⚠️ **Budget: the ELEVENTH raise** (`grep -c '^# RAISED' tests/lint/size-baseline.tsv` → 11; a draft of this entry said tenth, in the release whose subject is stale literal counts). A shrink was attempted and not found, and the attempt was narrower than first claimed: review-changes.md's 12 prose paragraphs over 700 b are instruction, but its two largest blocks are **fenced bash of 8,441 b and 2,438 b — 26% of the file** — which the search did not address. **H-022 direction: NEITHER bucket.** Confirm needs three or more raises for new content *without* an attempted shrink; refute needs shrinks *found*. One raise, shrink attempted, none found — neither. ⚠️ **H-022 already carries a paragraph headed *"Direction, and it is NEITHER bucket — two drafts of the raise note claimed opposite ones."* This entry was the third draft to claim a bucket the rule does not license, and the third caught by a review lens rather than by re-reading.** That recurrence is worth more than the bytes.

⚠️ **What rule 8 cannot see.** It measures bytes of adopter-facing surface; #127's benefit is tokens per review round. From `memory/review-ledger.tsv`, re-derived 2026-09-12: per-round sums run **121k to 485k tokens**, the largest being the four-lens #133/#140/#149 round at **485,021**. ⚠️ **The widely-quoted 557,442 figure appears nowhere in the ledger** — it is an adopter's session recorded in a comment on #126, not this repo's, and two drafts of this entry cited it as ours. **This release's own review round cost 359,568 tokens across three lenses** and found five blockers, which is the cost side of the same argument stated honestly.
...
⚠️ **Adopter action: re-install `curate` and re-copy `review-changes`.** Both shipped skills changed — `git diff v1.38.0..HEAD --name-only -- templates/ adopt.md README.md docs/GUIDE.md` returns `docs/GUIDE.md`, `templates/curate.md`, `templates/review-changes.md` and nothing else. `curate` is user-global, so a stale global copy is the one every repo loads; `review-changes` is project-local by design, so each repo carrying it needs its own re-copy. An adopter who does neither keeps the defects below — including a frontmatter skip that silences whole well-formed files.

**The adopter batch: four reported defects on two shipped skills, every one reproduced here before it was touched.** #150 parts 2–3, #151, #142, and the decidable half of #141. Each was measured against a **5,168-file, nine-repo estate** rather than against this clean tree, per the hostile-repo rule — ⚠️ but see the BOM paragraph: for one of them the estate population is **zero**, so that measurement is a control on the others and not evidence for it.

⚠️ **#150 part 2 was NOT fixed. It is now documented, and the reason is three refuted fixes.** A fenced block indented four spaces is scanned as markdown, because `sub(/^ ? ? ?/, "", bare)` strips at most three leading spaces and four is the ordinary shape inside a list item. Every attempt to widen that bought a worse class, each found by the review round rather than by the estate:
...
| bound the close relatively (`ind <= find + 3`) | a top-level indented **code block** containing an unpaired marker opened a fence and **silenced the whole file**, losing a lossy row the shipped version reports |
...
The class being fixed has **zero instances in the 5,168-file estate**, and it is a *visible* false positive; two of the three fixes traded it for silence. Reverting restores output **byte-identical to the shipped version across all 5,168 files**. So the step now documents the class in "Known blind spots" — which is what part 2 asked for, since it noted the class was in neither list — and **T11 pins the false positive as a test**, so a fourth attempt has to argue with a fixture rather than with prose.

**A leading `---` is a valid thematic break, and the frontmatter skip ate whole well-formed files (#151 part 1).** The skip opened on line 1 alone, so a file beginning `---` then a blank line then a heading reported *"no check ran on any line"* while a genuine lossy row went unexamined — the **silencing** direction, which is the one a denominator guard must not have. Line 1 now only *arms* the skip; the first non-blank line decides, since real frontmatter opens with a YAML key.

⚠️ **Three corrections to the fix as filed, each measured.** Confirming on line 2 alone reports Obsidian's own frontmatter as a malformed table (Obsidian writes `---`, a blank, then the key); it does the same to the **10 estate files** that open with a `#` YAML comment; and it does the same to a quoted key. So blank lines **and YAML comments are scanned past** rather than deciding, and a quoted key counts as a key. Populations, stated because a no-change result over an empty one proves nothing: **1,166** estate files open with a bare `---`; **1,162** carry a plain key on line 2; **4** change behaviour, **2** of them the Obsidian shape and **2** the same blank-then-key form in `.specify` templates — all benign.

⚠️ **The residual cost is a FALSE POSITIVE, not lost detection, and the first draft described it as the latter.** A frontmatter whose first deciding line is a block sequence, a `%YAML` directive or a key containing spaces is not recognised, and its closing `---` can then report as a delimiter row — the #52 class the skip exists to prevent. `#` cannot be made to decide, because a YAML comment and a markdown heading are the same bytes, and the heading case is what #151 is about. Both directions are now in "Known blind spots". Seeded as **T10** (asserted with an exact message, since the pre-fix behaviour also printed something — the wrong thing), **N10**, **N11** and **N12**, with ablations **A7**, **A8**, **A10** and **A11**.

🔴 **The BOM fix shipped broken to `master` and CI caught it — the clearest case yet for why CI is a different instrument and not a slower copy of the local run.** The first form was `substr($(0), 1, 3)`, which counts **bytes** in mawk and **characters** in gawk under a UTF-8 locale, where a BOM is one character. Local: 15 fixture suites and 13 lint rules green, plus a review lens asserting the guards *"behave identically under mawk, nawk and busybox awk"* — an unmeasured absolute that omitted gawk **because there is no gawk on this machine**. CI has one, went red on the push, and took six ablation kill-sets with it. Now `sub(/^\357\273\277/, "")`: a regex matches the same bytes under either reading and hard-codes no length in disputed units. ⚠️ **The fixture cannot catch this locally by construction** — it runs the extracted program in the ambient locale — and its `N9` comment now says so, because a green local run is not portability evidence for that row.

**A UTF-8 BOM defeated the skip outright (#151 part 2).** `NR == 1` cannot match `\xef\xbb\xbf---`, so the exact false positive the skip exists to remove came back. Stripped now, in octal rather than `\x`, which is a gawk extension. ⚠️ **The estate holds zero BOM files** — a count reported from a tree that is not in this repo, so no reader here can re-derive it; a positive would have been a path printed by a scan for a leading `EF BB BF`. So a real-tree run cannot tell this fix from a disabled one. ⚠️ **And `N9` is the whole evidence, not `N9` and `A9`: this release's own CI log shows `A9` PASSING on the run where the strip was broken.** `ablate()` scores a case by empty against non-empty, so when the unmutated program already reports on `n9` the mutant reports too and the sets match. `gh run view 34091734140 --log` on `64f737b`: `FAIL n9_bom_fm.md reported [...]` beside `PASS ablation A9 drop the BOM strip fails exactly [n9_bom_fm.md]`. **An ablation cannot fail when its own kill-set is the case that is already reporting** — the scope clause matters: the same run shows `A1`/`A4`/`A5`/`A7`/`A8`/`A11` failing *because* `n9` leaked into their sets. `A9` shows `N9` is load-bearing *given* a working strip, and nothing more.

**Two stale hedges and one false claim (#150 part 3, #151 part 3).** Frontmatter *at* line 1 no longer reports, so it is out of the false-positive list. The lone-CR hedge warned about the noisy half and not the quiet one: with the fence as the file's first construct a correctly closed fence reports as unclosed, but **with a heading above it the file goes entirely silent**, losing a row the identical LF file reports. Both measured. And v1.36.1's note that Step 1.5 *"catches this exact shape"* was **false for the rule it states** — the guard needs two risky tokens and the one-token form is what prettier 3.8.1 corrupts. The note now says backstop, not coverage.

⚠️ **The emphasis guard was NOT widened, and the measurement is recorded so the decision can be revisited rather than re-derived.** Relaxing it to one token costs **33 new hits over 5,168 files**: **19** sit in adopter *installs* of this same skill across 8 repos, where the shape is described in prose — ⚠️ **zero of them are in this repo**, so a run here measures 2 new hits, not 33, and the estate is the only instrument that sees the rest. Of the **14** outside those installs, **11** carry the risky token inside a bold run opened on the same line — the genuinely corruptible shape — while **3** are cross-line bold continuations, the broadening this guard already rejected. That is a precision change to a judgement instrument and belongs in its own issue with its own fixture, not folded into a message fix.

**`curate` reported an assertion of ABSENCE as a dead reference (#142).** `audit-context` Step 4 has skipped that class since v1.15.0; the extractor had no equivalent arm, so **two shipped checkers gave the same input opposite dispositions** — reported by an adopter who hit it twice, the second time in the text they wrote to record the first. The arm now covers all three shapes, and **three spellings of the negation** rather than the one its sibling still handles (#155).

⚠️ **Span-scoping is the load-bearing half, and the first draft got it wrong in the one place the sibling checker had already measured.** Two things are needed. A fragment is skipped only when **every** occurrence of it sits inside a marker — otherwise the same path struck once and referenced live once is silenced, which is how a real deletion hides. And each marker binds **its own span, never its line**: the first draft matched `**Deleted**:` to end-of-line, so *"`docs/retired.md` — use `docs/replacement.md` instead"* silenced the live replacement too. `audit-context` states that rule with its own measurement (line-scoping dropped 4 references on 2 lines in one adopter repo, 3 load-bearing), so the draft **re-created the exact divergence #142 exists to remove**, on the case the sibling had already paid for. Both are seeded true positives now, with `absence-arm-unscoped` and `deleted-marker-line-scoped` proving each row can fail.

⚠️ **The "three spellings" claim was a comment, not a measurement, for one round.** Only `! test -f` was seeded; collapsing the alternation to that one spelling left the whole suite green. `test ! -f` and `[ ! -f` are seeded now, and `negation-one-spelling` ablates them. Prose absence (*"no `docs/GUIDE.md` counterpart here"*) stays undecidable and is documented as a convention: drop the backticks.

**#141 is fixed only where it is decidable, and the refusal is the finding.** `ExecStartPre=wait_for_edh.sh` reached DEAD because the cross-repo arm keys on a first segment an unqualified token does not have; a `key=value` token with no `/` is now skipped. ⚠️ **The proposed sibling-uniqueness gate was refused on a measurement.** `tests/fixtures/dead-reference/` already carries the seeded true positive that decides it — a bare `package.json` that must stay DEAD — and the gate would pass that row **only because the synthetic estate has no sibling `package.json`**. The real estate has **3**, and **35** siblings hold a `README.md`. So the fixture would go green while the fix broke that exact class in the field: a fixture asserting the hole is correct, caught before shipping rather than after. #141 stays open.

**`docs/GUIDE.md` gains the `effort:` frontmatter note.** In the 2.1.263 bundle the key sits in the same frontmatter string table as `allowed-tools` and `disable-model-invocation`, and an effort enum occurs elsewhere in the binary. ⚠️ **That is an adjacency in a string table, not a schema this repo has exercised**, and the two facts come from different places — so the key ships as probable and the value list as unconfirmed. The consequence that *is* checkable: a pin becomes part of the bytes `install-global-skills.sh --check` compares — **against the tracked working-tree copy, not a release tag**. ⚠️ A first draft had this backwards in both files. Releasing does not *clear* a pin's drift, it *causes* it: on an unreleased tree the tag branch excuses the mismatch as `nothing to refresh yet`, and only once the tag moves does `--check` report drift, until the global is reinstalled.

**The templates budget was RAISED TWICE in this release — the eighth and ninth raises in this ledger.** ⚠️ **"Against one shrink" is dropped from this count deliberately**: `grep -c '^# ratcheted down' tests/lint/size-baseline.tsv` → **5**, and H-022 never defined whether a ratchet-down is a shrink for its denominator. Say raises, which is counted, not shrinks, which are not. Re-derive rather than read this paragraph: `grep -c '^# RAISED' tests/lint/size-baseline.tsv` → **9**, and `git show v1.38.0:tests/lint/size-baseline.tsv | grep '^# BUDGET'` → **249600** against **257,614** at the release commit, so the release-level delta is **+8,014**, not the +7,317 of any one raise. The eighth (2026-09-06) is +7,317 b, re-derived per file with `git show HEAD:<path> | wc -c`: `review-changes` +4,028, `curate` +3,289, then ratcheted down to 257,360. The ninth (2026-09-07) is **+275 b** and its trigger was **CI, not a lens** — the comment recording that the BOM strip must be a `sub`, on the shipped surface, because there is no gawk on this machine to re-derive the reason from. Then **ratcheted back down three times, to 257,614**, when the review below paid for its own correction out of the comment it was correcting, rather than taking a tenth raise — the second time in nine raises the hatch was available and declined (H-022). ⚠️ **Two drafts of this paragraph were stale in the same way, an hour apart.** The first said *"the eighth raise … all dated 2026-09-06"* and did not mention the ninth at all, written before the CI fix and never re-derived after it. The second quoted **257,635**, correct when typed and false within the hour because the ratchet that paid for the next fix moved it; a third draft quoted an intermediate that no artifact now holds, which is worse — unverifiable rather than merely stale. **A number is stale the moment the thing it measures is edited again, including by the edit that cites it.** ⚠️ **Two drafts of this paragraph got H-022 wrong in opposite directions**, which is worth more than the raise. The first called it *"evidence FOR"*; the second called it *"refute-leaning"*. H-022's rule is: *"Three or more raises for NEW CONTENT without an attempted shrink confirms it. Zero raises, or raises confined to correctness fixes **with shrinks found for the rest**, refutes it."* A shrink was **attempted and refused** — `curate.md`'s five largest passages are instruction, not archaeology — so no shrink was *found*. The refute condition has two conjuncts and only one holds; the confirm condition does not hold either. **This raise falls in neither bucket, and saying so is the honest entry.** ⚠️ H-022's own revisit trigger — *"any release in which `--raise-budget` is used twice"* — has fired, and this raise alone exceeds the previous seven combined.

Paid on the other budget: `CLAUDE.md` **−190 net** (39,749 → 39,559 b). ⚠️ **Two drafts of that ledger note carried wrong numbers, and the second is the more instructive.** Draft 1 said **−344**, which is the gross of the five `templates/` rows alone (all eight are 1,090 b) and not a net change at all. Draft 2 said **−271** — *correct when written, and stale within the hour*, because `CLAUDE.md` was edited again afterwards. That is through-line bullet 7 happening inside the paragraph that cites it: a number copied to a second surface stops being checkable against its source. Re-derive at commit time; never recall. Eight Key Paths rows went: **five** duplicated their Architecture-tree entry — two verbatim, two where the tree line additionally carried a version, and one (`templates/project-file.md`) where the tree line is the *shorter* paraphrase, so a little description was lost rather than deduplicated — **two** were merged into rewritten tree entries, and **one**, `tests/lint/skill-sync.sh`, was dropped with its fixture pointer restored to the tree line in the same change.

**`CLAUDE.md`'s fixture counts become commands, having been wrong before this change touched them.** It claimed *"40 rows, 16 ablations"* for `dead-reference` while the file at **`v1.38.0`** held **35** and **15** — #93's class, fourth instance, found by measuring rather than by reading. ⚠️ **A draft of this sentence said "at HEAD", and HEAD moves**: `7dd50d3`, in this release, took the same two commands to **46** and **21**, so a reader running them at v1.39.0 matches neither number. A count pinned to a moving ref is the defect this paragraph is about, committed in the sentence reporting it.
...
🔴 **Step 5's own statistic was stale on the shipped surface, and the instrument behind it was missing a whole session.** `templates/review-changes.md` prints the command that re-derives its numbers; running it refuted the prose beside it — the skill said **14 introduced / 178 findings / 52%** where the ledger gave 15 / 190 / 54%. ⚠️ **Then the ledger itself turned out to be the bigger defect**: the session that ran five lenses on the evening of **2026-09-06** and committed them as v1.39.0 the next morning appended **none** of those rows, including the round-2 pass whose **nine findings were nine-of-nine `introduced`** — the largest single contribution this statistic has ever had. Reconstructed from the subagent transcripts (exact `calls` and `seconds`; `tokens` taken from the transcript rather than the task notification every other row used, which the row notes disclose along with the ±4% that costs). Re-derived once, from the completed ledger: **24 introduced against 13 missed — 65% of those pairs — and 24 against 219 counted findings, 11%.** The shipped figure moved from *half* to *most*. **A ledger that skips the session with the worst result flatters the thing it measures**, and the skill quoting it had been wrong through v1.38.0 as well.

**`docs/seeded-defects-and-ablations.md` gains the failure mode this release found: an ablation cannot fail if its kill-set is already failing.** A typical ablation scores *reported* against *silent*; when the unmutated check is already reporting on the seeded case — because the guarantee is broken — both original and mutant report, the sets match, and the ablation prints PASS **because** the thing it guards is broken. The CI log has the FAIL and the PASS four lines apart. Two rules follow, and they are adopter-facing: **read an ablation's PASS together with its own seeded case**, and **read ablation results only over an otherwise-green suite**. This is the sibling of the co-seeded-control rule already on that page, and the sharper one.
...
⚠️ **The headline is not the bug count, it is what the bugs cost to find.** This release's own review found **thirteen defects, four of them blockers**, in work that had already passed twelve lint rules, thirteen fixture suites, CI and the author's read — and one of the fixes for those was wrong in the opposite direction within the hour, caught by CI rather than by the local run. The base rate this repo keeps re-measuring is unchanged.

⭐ **The finding that changes policy**: `memory/review-ledger.tsv` shows **half of classified review findings were defects the previous round's own fixes created**, not misses. So the cost of the review loop is driven by the defect rate of *fixing*, and a round cap on its own ships the introduced defects. `templates/review-changes.md` gains **Step 5** for that.

### The review of this release found thirteen defects in it, four of them blockers

Two independent cold lenses, one round, on the whole session diff (1,082 insertions, 25 files, 14 commits) — a doc-accuracy lens and a correctness lens, run cold rather than as forks precisely because a fork inherits the blind spots that produced the diff. **Everything below had already passed twelve lint rules, thirteen fixture suites, CI, and the author's read.** The two lenses returned **disjoint** findings, which is the second measured instance of that this session and the reason the new Step 5 forbids collapsing lenses into the author's context to save tokens.
...
**The second lens found two more blockers, and the first is a regression this release introduced:**

3. ⚠️ **The `#108` source-extension list broke real runtime state.** `.ts`, `.js`, `.php`, `.c` and `.h` were in it, and its comment claimed the only residual gap was a generated `data/schema_pb2.py`. **That claim was unmeasured and wrong.** Three ordinary real-world paths regressed from `runtime state` to a bare `UNRESOLVED`: `var/cache/prod/App_KernelProdContainer.php` (Symfony's compiled container), `data/segment0001.ts` (MPEG transport stream, not TypeScript) and `cache/bundle.js` (bundler output). Worse, their heads — `var`, `data`, `cache` — exist locally, so **#118's new hint cannot fire for them either**, and the adopter gets the unactionable row #118 was written to reduce. **An extension whose meaning depends on the ecosystem is not evidence of source.** The list now holds only extensions with no common data or build-artifact meaning; N37 seeds all three, and restoring the broad list fails it while leaving T32 green.

4. ⚠️ **Rule 1's no-work-tree skip exempted `.claude/skills/`, which the comment three lines above says is never exempt.** `.claude/skills/` is *negated* in `.gitignore`, so it is tracked and present in a tarball — a dangling pointer under it is a real break whether or not git can be asked, and it scanned clean. **And the coverage line printed "exempt (absent AND gitignored)" when no `check-ignore` had run**, asserting a test that did not execute. The skip is now scoped to `memory/` alone, and the line reads `UNVERIFIED — rule SKIPPED` when it is.

⚠️ **The fix for blocker 4 was itself wrong, and CI caught it — not the local run.** Refusing to exempt the whole `.claude/` arm under the skip re-created #125's original symptom for a different path: a **false FAIL on `.claude/settings.json`**, which *is* gitignored and legitimately absent, and which is precisely what the skip exists for. The arm splits in two — `.claude/skills/` is never exempt because it is tracked; everything else under `.claude/` is. **Both halves were got wrong once**, in opposite directions, within one hour.

⚠️ **And two ablations went inert as a side effect**, each refusing rather than passing: A5's mutant anchored on `^printf`, which the new if/else indented, and A3's expectation widened because its unanchored mutant strips every skip counter. *"Changed nothing"* is the report that makes an ablation worth having — a mutant that silently applies to the wrong line still says PASS.

**One more, from the same lens:** `_` had been added to the whole-token boundary class, which **ate markdown emphasis** — `_alpharepo_`, ordinary italics around a repo name, stopped marking the sibling it names. A repo name containing an underscore is rarer than italics around one, so `_` is a separator again. N38 seeds it.

**Three accepted and documented rather than fixed**, because each is a real trade rather than a defect: an intra-sibling `COLLISION` short-circuits before later siblings are examined (both outcomes say *no single provenance*, which is the actionable part); skipping same-repo PR runs means `refs/pull/N/merge` is never tested, which is safe only because this repo merges locally — ⚠️ **and would be wrong twice if these ever became required checks**, since a required check that never runs sits pending forever; and `--raise-budget` used to rewrite the `# SPILL` line, **erasing a same-commit moving-bytes payment before anyone could see it** — a raise silently laundering exactly the move #131 exists to surface. That last one is now reported before the baseline is rewritten.
...
**#109 — one file of the auto-loaded set.** Step 0.8 checked the project file alone. Claude Code auto-loads the project file *and* a user-level memory index. Measured on an adopter: 31,613 + 9,941 = **41,554 chars, over the 40k hard cap**, while this step read 31,613 and reported *"comfortably under the 35k soft target"*. It had been wrong for roughly a fortnight, on every session, and was caught only because that adopter had recorded the pair by hand in an earlier audit — nothing in `curate` would have surfaced it. The step now sums **every file the tool loads without being asked**, and requires the set to be listed before it is measured, naming anything deliberately not counted.
...
**#119 — a hyphen defeated the whole-token rule.** Bullet 5 requires a sibling's name to match as a whole token; the boundary class was `[A-Za-z0-9]`, so `-`, `_` and `.` read as *separators*. Prose naming one repo therefore marked a different repo sharing a hyphen-delimited component, and a reference resolved against a neighbour the prose never mentioned. `-` and `_` now count as part of the name. **`.` deliberately does not**: counting it would stop a sentence ending "we use alpha." from marking `alpha`, which is the common case. The knowing cost is a miss — "alpha-based" no longer marks `alpha` — and a miss is the safe direction.

**#120 — the unmarked arm never reported cross-sibling ambiguity.** It stopped at the first named sibling with a hit and stated it as fact, while the **marked** arm reported the ambiguity on identical input. The asymmetry ran backwards: *marking* a reference made the tool more careful about it, and leaving it alone — overwhelmingly the commoner form — got a guess resolved by sort order. Line 99 of the step already required ambiguity reporting of both arms. Both now do it.

⭐ **The fixture's own X10 comment had recorded both bugs, from the other side, without either being filed as such**: *"a token that appears in 13 lines of this fixture's prose, since a hyphen is a token boundary and every mention of `sibling-repo` contains it. It sorted first and the rung-4 loop breaks on the first hit."* A neighbour leaked into the search, a probe document flipped from a reported break to a clean resolution, and **nothing failed**. X10 was written to assert the neighbour *count* — treating the symptom — while both causes stayed live for months in a sentence describing them exactly.

**Seeded and ablated, in an isolated estate** so the added siblings cannot change the main run's neighbour count and trip X10. T30 (#119), T31 (#120), and **N33 as the control** — a sibling named whole-word in bare prose must still resolve, without which "never mark anything" would score 2 of 2. Reverting each fix fails exactly its own case and leaves N33 green.

⚠️ **Lint rule 12 caught a private project name being committed, in this change.** The reproductions were lifted verbatim from the issue bodies, which name real repositories in a private estate; this repo is public. Renamed to synthetic (`alpharepo` / `beta-alpharepo`) before the commit. **That is rule 12 doing precisely the job it was built for, one release after it was built**, and it fired on the author of the change rather than on some future contributor.

### A fixture needle that matches two findings tests neither (#116)

`tests/fixtures/reference-integrity/` declared T12 as `"...|src/models/temporal.py"` — the needle was a **path, not a reason**, so any other seeded case referencing that file satisfied T12 without T12's own subject being reported at all. Measured on the withdrawn #76 branch: with a second document naming that path, disabling the arm T12 exists to test still produced `PASS T12`. **The fifth instance of this collision in one fixture.**

T12 and T13 now assert their reason strings, which differ from each other (`STALE PLACEHOLDER MARKER` vs `PLACEHOLDER SHAPE THAT RESOLVES`) and from T19/T21's rung 4. **Proved live by ablation**, not by the suite turning green: disabling the rung-1 mislabel arm now fails both.

**The recurrence is fixed, not just the instance.** The runner now asserts that no two T-cases share a needle, and that each needle matches **exactly one** reported line — because a needle satisfied by a second finding cannot distinguish its own subject, which is the shape all five instances had. Both arms were ablated: duplicating a needle fires the first, reverting T9 to its old path needle fires the second.

⚠️ **The guard refuted its own author on first run, and the comment it refuted was already written.** A draft of that comment claimed *"every needle matching exactly once"*. Only exact duplicates had been measured (0 of 23); match counts had not. The guard found **three** more instances immediately — T4, T9 and T14, taking the running total from five to eight. T9 had a clean fix. T4 and T14 do not: their extra matches are the *same reason* reported from a different seeded document, and a doc-qualified needle would key on the report's column padding, which moves with path length — the hazard T29 already failed on. **They are DECLARED weak in the runner rather than silently tolerated**, per rule 11's principle that an exemption is declared, not guessed. A declared row buys nothing for its own discrimination; it is a marker so the next person does not rediscover it as instance nine.

### The method's most-measured invention finally ships to adopters (#130)

`ablat` appeared in **18 files under `tests/`, 1 template, 1 `CLAUDE.md`, and 0 files under `docs/`.** The part of this framework with the most measured catches — seeded-defect fixtures, and ablations that prove a check can still die — reached adopters not at all, while the layered-memory story that leads every adopter-facing surface remains an open hypothesis (H-017).

**New: [`docs/seeded-defects-and-ablations.md`](docs/seeded-defects-and-ablations.md).** Tool-agnostic, no harness required. It teaches the two instruments, opens with four checks *from this repo* that were green while broken (a rule matching 0 of 4 references for its whole life; a template-vs-install comparison passing for eight releases over a shell syntax error carried identically by both copies; a test family never run against the system it claimed to test; a guard no input could make fire), and gives a four-step minimum version whose fourth step — break the check on purpose and confirm the fixture goes red — is the one people skip.

⚠️ **It ships the limits with the method, because a fixture is the easiest thing in this framework to mistake for a certificate.** Recall against seeds is a lower bound, never proof: three drafts of one check here each passed their own fixture and were each refuted by a real corpus (28 false hits, then 15, then 1). External corroboration, added because the internal version could be dismissed as local: Just et al. (FSE 2014) found only ~**73%** of 357 real faults coupled to standard mutation operators; a 2026 evaluation measured LLM reviewers at **F1 0.847 on synthetic mutants against 0.066 on real PRs**; and the software-inspection literature on defect seeding reached the same warning decades earlier — estimates are biased unless seeded and natural defects are equally detectable. The sharpest warning is the one this repo earned twice: **a fixture written by the author of the change it justifies, for a loosening, contains zero true positives and cannot measure sensitivity at all.**

⭐ **Linked, not just written.** `docs/verification-rationale.md` and `docs/verifying-what-we-write.md` were reachable from **nothing** — not `README.md`, not `docs/GUIDE.md`. Adding a third orphan would have reproduced the exact failure this issue names, so the new page is linked from README.md's Further Reading and from `docs/GUIDE.md` § Verification Hooks, and `verifying-what-we-write.md` was linked at the same time.

**And the rule catalog was missing a quarter of its rules.** `CLAUDE.md` sends readers to `tests/lint/README.md` as "the rule catalog"; it documented rules **1–9** while twelve exist. Rules 10 (an ablation that cannot kill anything), 11 (a fenced bash block an adopter copies must parse) and 12 (no private project name in a tracked file) are now in the table, with the stderr-coverage contract that rules 10 and 12 depend on stated explicitly: a checker that could not run emits exactly the same empty stdout as a clean one. Found while verifying a claim for the new page — the claim was right and the catalog was not.
...
|---|---|
| **a defect the previous round's own fixes created** | **50%** (14 of 28) |
| a defect earlier review had missed | 21% (6) |
| first sighting | 29% (8) |

⚠️ **Population, stated because the first draft of this entry got it wrong twice.** **4** real-work rounds, not 5 — the awk that produced the figure skipped a leading comment with `NR>1` and counted the file's own **header row** as a specimen; the sums were unaffected, so every number was right while the sample size was silently inflated. And this is half of the **classified** findings: 15 further findings sit in rows whose origin columns are `?`, which the ledger header says means *not guessed*. Against all 43 counted findings the share is **33%**. Both are stated; the honest one turns on a question the ledger cannot answer, and the largest excluded row is round 5 of 7, where introduced defects are most likely — so the exclusion is **not** conservative.

One round produced 5 of the next round's 8 findings; the rounds on that release reached **seven**. **So rounds appear to multiply because each round's fixes need another round to check them, and a round cap may save nothing on its own — it can ship the introduced defects instead.** The cost is in fixing, and this framework's adopter-facing surface had *no guidance on fixing at all*: `review-changes` ended at "report", and every one of the twelve lint rules and thirteen fixtures points at the authoring side.

**New Step 5** states four fixing rules — a fix is a change and takes its file's tier; re-read the steps that consume what you changed; fix one finding at a time within a file; name what the fix could have broken before starting another round — and four budgeting rules, of which one is a warning against the saving this repo nearly adopted.

⚠️ **Two recommendations were drafted and killed before shipping, and both are recorded because the second is the instructive one.**
1. *"The bench will price the review."* It will not. `tests/fixtures/review-bench/` was already run (6 runs, 2026-08-27) and H-016's own Caveat 2 says it **cannot measure cost** — both configs cost ~42k on a three-file corpus. The prescribed run would have bought a known-invalid number. Three memory sites and #126 asserted it had "never been run for its purpose"; the ledger refutes that in six rows.
2. *"Collapse the lenses into one reviewer to save tokens."* **Refuted by the engineer**: *"different context seem to give truly different views, and may improve the results."* A reviewer holding the author's context holds the author's blind spots — collapsing lenses is the self-certification failure this skill exists to prevent, with a cost justification attached. Measured here the same day: on one diff the shell lens and the adversarial lens returned **disjoint** findings. Step 5 now warns against the saving explicitly.
...
Twelve lint rules and twelve gating fixture suites had existed for months and gated **nothing** — counts checked, because the issue that filed this said eleven and thirteen, and both were wrong in a paragraph arguing that unrun checks rot: `CLAUDE.md` said to run them, running them was something someone had to remember, and *which* of them to run was the author's choice — the anti-self-certification shape one level up. #105/#112 is what that produces: a shell syntax error in a block adopters copy, shipped through eight releases, in a repo with a green suite. `.github/workflows/checks.yml` now runs `tests/lint/run.sh` and the new `tests/run-fixtures.sh` on every push and pull request. Closes #115.

⚠️ **The lint could not run in CI at all, and nothing said so because nobody had ever run it there.** On a fresh clone — which is exactly what a CI checkout is — the first two rules behaved like this:

| Rule | On a fresh clone, before this |
|---|---|
| 1 — `CLAUDE.md` paths resolve | **Four FAILs, every one false.** `CLAUDE.md` correctly references `memory/MEMORY.md`, `memory/gotcha-log.md` and two topic files, all gitignored maintainer-local state |
| 2 — `memory/MEMORY.md` index integrity | **Nothing.** `grep` wrote *No such file or directory* to stderr, the loop read an empty stream, zero failures were recorded, and the run printed *All lint checks passed* |

Rule 2's silence is the trap rules 6 and 12 are both built around — an absent population and a clean one producing byte-identical output — sitting two lines above them, in the rule that runs first. Rule 2 now reports a **SKIP** and counts it; rule 1 exempts a `memory/` path only when **the whole directory is absent** — the fresh-clone shape — and **prints how many it exempted**. CI's own run on the final commit (`f106d23`): `27 file and 17 directory reference(s) checked; 4 + 1 exempt` and `2 rule(s) SKIPPED — a skipped rule is not a passed rule`, with the skips surfaced to the run summary rather than folded into the pass.

⚠️ **Then the coverage line found a second one, in the arm nobody was looking at.** `memory/MEMORY.md` writes every pointer as `` `memory/project_x.md` ``; rule 2's pattern anchored a backtick directly before `project_`, so it extracted **0 of 4** — that arm had matched nothing since it was written. It was green from both sides: the orphan loop greps the basename and passed independently, so no failure ever appeared and no output ever suggested one. Printing the count is what exposed it, one commit after the count was added for a different reason. Both rules now **fail on their own zero coverage** — an absent or reference-less `CLAUDE.md`, or an index with topic files on disk and no extractable pointer — where before each printed a zero and exited 0. Rule 2's gate is scoped to *topic files exist but none was extracted*, deliberately: an index that legitimately has none is not a defect, and inventing one to avoid a silence is not a trade this rule makes.

**That exemption is a loosening, so it ships with seeded true positives.** `tests/fixtures/clone-lint/` **extracts** rules 1–2 from `tests/lint/run.sh` rather than copying them, and drives them over fifteen seeded repos and eleven ablations with exact kill sets. Three cases exist only to catch an exemption that is too generous: a missing file under `.claude/skills/` (tracked by negation, never exempt), an absent `memory/` that is *not* gitignored, and a gitignored path outside the allowlist. Ablation A1 kills **two** of them rather than the one predicted — `.claude/skills/` is saved only by `check-ignore` reading `.gitignore`'s negation, which is invisible in the rule's own text, and the fixture's exact-kill-set assertion is what said so.

**The workflow was verified by making it fail**, per the seeded-true-positive rule applied to CI itself: a scratch branch carrying one seeded defect per job (a dangling `CLAUDE.md` reference; rule 1's exemption with its `check-ignore` test removed) turned **both jobs red** — `structural lint` on `FAIL CLAUDE.md references docs/seeded-does-not-exist.md`, `sensitivity fixtures` on `clone-lint … SENSITIVITY REGRESSION`, with T2 and T3 failing exactly as A1 predicts. Branch deleted; the run stands as the evidence. A green tick that has never been shown to go red is not a gate.

⚠️ **The first form of that exemption was refuted, and it had given away the thing the rule is for.** Testing per-*file* absence looked right and was not: `/memory/` is gitignored wholesale, so **every** absent `memory/` reference was exempt on a fully populated checkout as well — including the stale pointer `/curate` leaves behind when it renames or archives a topic file, which is this rule's load-bearing class and is still advertised as such in `CLAUDE.md`. Measured on the live repo: appending `` `memory/project_TYPO_does_not_exist.md` `` to `CLAUDE.md` scanned green with `memory/` fully present. The exemption now requires the whole directory to be absent; `.claude/` cannot use that test (the directory exists in every clone, because `.claude/skills/` is tracked), so it stays per-file and **the residual gap is declared in the rule rather than hidden**.

⚠️ **And the fixture written to justify the loosening was asserting the hole was correct.** Five of its seeded repos carried a dangling `memory/` reference on a populated checkout; none asserted on it, and `N2` asserted the result was *clean*. Applying the correct fix turned the fixture red — a sensitivity fixture rejecting the tightening it exists to protect. That is the seeded-true-positives rule failing in the exact way it warns about: the cases were the ones the author thought of, drawn from the same mental model as the change. `T12` is now that case, `A10` is the mutant that restores the old form, and `A2`'s kill set turned out to contain `N2` as well as `T7` — the two arms of the exemption are not independent, which is visible only from an exact kill set.

**The review round found four more, and the fixture found one in itself.** A shell-correctness pass over the diff reported that rule 1 could still pass having checked nothing; that the directory arm exempted `memory/` on every CI run with nothing counted or printed; that the CI step summary printed *"no rules skipped — all twelve ran"* over a `lint.log` that `tee` creates **before** the script runs, so a run that died on line 1 rendered as full coverage; and that `tests/run-fixtures.sh` reported `ok` for a suite that exited 0 having printed nothing. All four are fixed and each is now asserted. Separately, ablation **A5**'s first form matched a *comment* quoting the coverage line and deleted the counter initialisation beneath it, killing all fourteen cases — **the exact-kill-set assertion is what said so**, which is the whole argument for asserting kill sets exactly rather than "this mutant fails something".

**No path filters, deliberately** — a stated deviation from the issue's own plan. Scoping each suite to the paths it covers saves about two minutes of a **2m12s** local run (37s on GitHub's runners, where the whole set is cheaper than the argument for trimming it) and buys back exactly the selection mechanism the workflow exists to remove. `tests/run-fixtures.sh` **enumerates** `tests/fixtures/` instead of listing it, so a suite added tomorrow runs tonight, and it refuses four silences: an empty population exits 2, a **gutted** one fails (deleting 9 of 13 suites otherwise printed *"4 of 5 … 0 failed. All fixture suites passed"* — the pre-#115 rule-2 silence rebuilt inside the file written to close it), a fixture dir with no runner FAILS unless declared a non-gate, a suite that exits 0 having printed nothing FAILS, and one red suite never stops the others. `review-bench` is that declared non-gate — it scores a review configuration and has no pass/fail by design; wiring it in would turn a measurement into a threshold nobody chose.

⚠️ **What CI does not check, recorded in the workflow's own header so it cannot be read off a green tick**: the cross-step contracts between skill steps (#113, still without an instrument), and anything requiring judgement — `/review-changes` is still the diff-driven step. **Two rules SKIP in CI and cannot do otherwise**, so the local run remains the stronger one.
...
**1. Lint rule 8 was a log wearing a budget's clothes.** Measured over its whole life, the recorded size of `templates/curate.md` moved **up 21 times and down 0 times**. Every growth was legal: the rule said *"shrink something, or record why in `CHANGELOG.md` and run `--update`"*, and recording why is free, so the second branch was taken every single time — by every author, including the one who wrote the rule, twice in the session that measured this. The unit is now the **total**: growth in one file is free if another shrinks to pay for it, exceeding the budget fails, **`--update` cannot raise it**, and raising takes `--raise-budget "<reason>"` recorded in the baseline. Falling below locks in, so the ceiling only ever descends. ⚠️ The old scan also globbed `templates/*.md` at depth one, so `templates/physics-tests/` — 19% of the surface — **was never measured by the rule whose entire job is measuring that surface.**

**2. `templates/physics-tests/` is archived (−59,935 bytes).** Its own README said it *"has never been run against a live simulator — not by this repo, not by any downstream consumer"*; #25 tracked exactly that and was closed without the validation arriving. #16 is the precedent. It moves to `docs/archive/` with a note saying what un-archives it: one report of it being run against real simulation code.

**3. Rationale moved out of skill bodies — and the size of that win was overestimated, including by this entry's first draft.** `docs/rationale/<skill>.md` now holds superseded drafts and the measurements that refuted them; nothing is duplicated, and a claim appearing in both means one is wrong. Measured first: the rationale is **not** in `CHANGELOG.md` either (2/15, 2/25, 0/8 verbatim echoes), so this is a move, not a de-duplication.

⚠️ **The "48% archaeology" figure that motivated this was an artifact of the instrument.** It counted any *line* containing `measured`, `refuted`, `#NN` or a version as archaeology — but those lines are overwhelmingly decisions with a measurement embedded in the same sentence. Hand-distilling the eight largest passages in `audit-context.md` yielded **1,722 bytes, net**, not the ~60%-per-passage the regex implied. **And the automated first pass moved two decisions out of the body** — the arm-dependent stale-marker remedy and the four stamp formats an adopter must handle — which is a worse failure than the bloat it was fixing; both are restored. The extractable fraction is the *narration* of superseded drafts, ~5% of the five skills, and it is now out. The rest is not fat.

**Net: `templates/` 311,924 → 244,822 bytes, −67,102 (−21%), back below the v1.25.0 level.** `curate.md` itself grew this session (61,365 → 67,089, the extractor work above) and that is now a normal event rather than a breach, because the archive paid for it — which is the trade the old rule pretended to ask for and never once enforced.
...
### `curate` Step 0.1 — six false-positive classes in the dead-reference extractor, and four more defects found by reviewing the fix (#104, #106)

Three shapes were filed. **Three more were found only by running the reproduction rather than reading it.** Then two review rounds found **four further code defects in the fix itself**, and refuted one of its measurements outright — that last one is the most useful thing in this section, so it is stated in full below rather than softened.

**Population for every number here**, stated once because two drafts got it wrong: **66 directories holding a `CLAUDE.md`** under one estate root at `find -maxdepth 4`, resolving to **51 distinct git roots**. An earlier draft called that "66 real repos", which overcounts roots by 15. ⚠️ And the runs are not one-per-document: the extractor resolves its arguments against the **git root**, so for the 15 nested directories the run re-read *the root's* project file. Read the aggregates as 66 runs over 51 roots, not as 66 documents. Absolute counts also drift by a row or two between passes, because some of those repos have live data directories and gitignored memory files that change during a session; the deltas are stable and the deltas are the claim.

| Fragment (the shape, with adopter paths neutralised) | v1.36.1 | Now | Filed? |
|---|---|---|---|
| `report-summary-{nl,en}.md` | `DEAD (resolves nowhere)` | `SKIPPED` — a shape | #104 |
| `docs/work-items/[slug].md` | `DEAD (resolves nowhere)` | `SKIPPED` — a shape | #106 |
| `../SiblingDir/notes/topics/live.md` | `DEAD ... absent in the sibling ..` | resolves | #106 |
| `./deploy.sh` | `DEAD ... absent in the sibling <parent>` | resolves | no |
| `/CLAUDE.md` | `DEAD ... absent in the sibling <parent>` | `CANNOT VERIFY: path on another host` | no |
| `C:\devroot\project\templates\registry.md` | `DEAD (resolves nowhere)` | `CANNOT VERIFY: path on another host` | no |

**Aggregate: `DEAD` rows 121 → 90, resolutions +27, directories reporting `0 dead` 26 → 35, `DEAD` set changed in 21 of the 66 runs, and zero new `DEAD` rows anywhere.** The repo that filed #106 goes `3 dead → 0 dead`. The 31 removed rows partition exactly: **8 dot-relative in 7 directories, 10 Windows-shaped in 8, 10 brace/bracket, 3 POSIX-absolute** — no residual. Separately, **19 `~`-prefixed references in 19 directories** now resolve instead of being reported.

**1. Four shape conventions, not two — and checked before quarantined.** The test was `<` and `*`; `{a,b}` is neither a placeholder marker nor a glob star, and `[slug]` is the third convention in this framework's own templates. Both reached the resolver and were reported in the loudest disposition. The arm now resolves the fragment as written before skipping it, because a shape can be a real name: `[slug]` is a literal directory in Next.js and SvelteKit (2 tracked files in one estate repo) and `<slug>.md` is legal on ext4. That pre-check has **zero instances in the estate** and comes from a review's constructed case, not an observed one.

⚠️ **A measurement in the first draft of this section was refuted, and the instrument was the problem — this is the most transferable thing here.** The draft said the brace skip "silences two real dead references", contradicting #104's judgement that the shape is *"rare enough that the skip is the better trade"*, and #121 was filed on that basis. The member check behind it asked `(root/member).is_file()`. **That bare root-relative check is #51's own false positive** — the one the extractor's basename rung exists to remove — so it reported members absent that the extractor resolves one directory down. Re-run through the real ladder: **11 brace fragments, 9 not already skipped by `<` or `*`, and every member of every comma-brace fragment resolves.** Measured cost of the skip: **nil**. Both fragments named as dead are on disk — one directory down from where the fragment named them, which is the resolution the basename rung performs and the bare check skipped — and the third candidate contains a `*`, so v1.36.1 skipped it too and it was never `DEAD`. **#104's judgement stands, on evidence it did not have.** Expansion would still be right on all of them, so #121 stays open as correct-but-unprioritised with the corrected numbers on it, and the fixture's seeded case is now labelled a constructed one. Census, for whoever picks it up: 7 comma-brace fragments and 4 metavariable (`{name}`, `{PROP-NNN}`) across all 11; 6 and 3 among the 9 the change newly touches. The bracket half cost nothing either — 4 bracket fragments, 1 not already caught by `<`, none naming a file that exists.

**2. `./` and `../` are doc-relative, not cross-repo.** `..` is never a top-level directory of the repo, so these reached the qualified-sibling rung, which then tested a path **one level above** the one the fragment named. A live file was reported dead — and the printed reason was the tell: *absent in the sibling `..`*. A sibling literally named `..` is a branch announcing it was never meant to fire. Resolution is lexical (`normpath`), because `..` in a document means the textual parent. Inside the tree the reference is decidable, so it is decided; outside, a directory on disk decides, and nothing on disk falls through to `CANNOT VERIFY` rather than to a false `DEAD`.

⚠️ **The arm resolved against the repo root as well as the document, and two independent lenses caught it.** For a `../` fragment the root base necessarily lands **outside** the repository, so a stray `RUNBOOK.md` beside the repo silenced a dead reference — in exactly the population this method creates (`memory/gotcha-log.md` exists in 35 git roots of one estate, `RUNBOOK.md` in 13). **Nothing here could have caught it**: deleting that base left every case row and every ablation green, so it was untested exposure, and it contradicted the arm's own comment one line above. One base now, with a seeded stray file, an asserted row, and an ablation that reintroduces the defect exactly.

**3. The `path on another host` arm was unreachable for every path it was written for.** It sat **below** the cross-repo rung for three releases. A POSIX absolute path always contains a `/` and its first segment is `''`, never a top-level directory here, so the rung claimed it first; pathlib discards the left side of an absolute join, so `root.parent / '/opt/x.json'` tested `/opt/x.json` and printed `DEAD ... absent in the sibling <parent>, which IS on disk` — a false `DEAD` whose stated reason names a sibling that has nothing to do with it. Windows drive and UNC paths arrived at the same place by the opposite route — no `/` at all, so the rung never saw them and the basename lookup found nothing. The arm now precedes the rung and covers both. It also checks the path before quarantining, because withholding a verdict the run **has** is the sibling rung's own sentence pointing the other way: **19 of the 23 resolutions this arm makes are `~`-prefixed.**

Three defects in that arm came out of review, two of them from the fix itself:

- **A symlinked repo read as another host.** `root` is resolved and `ap` was not, so on a repo reached through a symlink — the default on macOS, where `/var` and `$TMPDIR` are symlinks — an in-repo absolute path missed the in-repo branch entirely. `ap` is now resolved **only when genuinely absolute**: a Windows fragment is not absolute on POSIX, and resolving it would join it to the cwd and land it inside the repo, turning a false reason into a false `DEAD`.
- **`ap == root` could only ever be true of the root itself**, which exists — so that disjunct asserted `resolves nowhere` about a directory that is there. Dropped, and a directory-valued path in either new arm now reports `names a directory, not a file` instead of being called dead.
- **The asymmetry with the doc-relative arm is deliberate and now says so.** That arm applies an on-disk-directory gate and this one does not, because a `../` fragment is unambiguously about the author's own tree while an absolute path is a claim about *a* filesystem that may not be this one. With the gate, `/opt/app/x.json` written on another machine would read `DEAD` wherever `/opt/app` happens to exist locally — a false `DEAD` invented by the environment, which is the one thing this extractor must not do.

**A fourth, outside both arms:** outside a git repository the root falls back to `.`, and `Path('.')` is a parent of `../x.md` — so every missing `../` fragment read as *inside the tree* and was decided `DEAD` in a directory that has no tree. `.resolve()`, with a non-git case and its own ablation, both guarded so that a `$TMPDIR` inside a git working tree skips them instead of accusing the extractor.

**The ablations are the argument, not the green rows — and this round is the evidence for that sentence.** The fixture now carries **40 case rows and 16 ablations**; each ablation deletes one arm or branch and must be **lethal to its own row while a control stands**, with the control in the same arm wherever one can be, since a control from another arm can only die if the mutant crashes. What the two review rounds found in the fixture itself is the part worth keeping:

- **Five of the new rows passed unchanged against v1.36.1.** `want` matched only class and fragment, never the parenthetical reason — and three of those rows were `DEAD` at v1.36.1 with #106's own tell, so the needle was satisfied by the mechanism the change exists to remove. That is the T12 collision in new dress. A `want_why` matcher now pins the reason string on those rows.
- **The `~` resolving branch had no case at all**, and the case added for it had **no positive assertion**: `resolves` passes on absence, so an empty capture — a crash, a typo in the `HOME` override — read exactly like success, which a lens proved by replacing the capture with a command emitting nothing. It now asserts a positive against the same capture first.
- **A guard that printed `SKIP` and then ran the rows anyway**, so one passed vacuously and two ablations went red accusing the extractor. It now actually gates them.
- **A dead matcher** left behind by its own replacement, and an `ablate` call that would have reported PASS for any mutant given an empty pattern, since `grep -qF -- ''` matches everything.
- **And the same class one more time, caught while writing this entry**: review added an `is_dir` branch to both new arms — a directory whose name carries a file extension is not a dead *file* reference — and it shipped with **no case**. Two rows and two ablations now cover it, in the arm where it would otherwise have been reported `absolute path inside this repo` and in the one where it would have been inside-the-tree `DEAD`.

The harness refuses a mutant that does not apply, one that changes nothing, one that kills its control, and one given an empty pattern. **Every one of those refusals was demonstrated by running it**, and the "changed nothing" branch fired three times on no-op mutants written while building it, plus twice more on stale expectations after the code changed under them.

⚠️ **`templates/curate.md` grew 61,365 → 67,432 bytes (+6,067), and the ratchet is updated deliberately rather than shrunk into silence.** Inside the extractor block that is **+1,215 bytes of code and +4,616 of comment**, after two compaction passes; the comments are what stop the next author from re-widening the quarantine, re-ordering the arms, re-adding the second resolution base, or re-taking the measurement with the wrong instrument. Adopters pay it every session. Recorded here because rule 8's own remedy is *shrink something, or record why in `CHANGELOG.md`* — the second path, with the number stated. The alternative considered and rejected was deleting other measured evidence from the same block to buy room.
...
**Size.** `templates/audit-context.md` grows 43352 → 45516 bytes (+2164) and the rule 8 baseline is updated deliberately. It buys a corrected rung order, the decidable/mislabelled distinction, the scope on two refuted absolutes, and a hedge on a third: *"rung 4 declines an angle-bracket segment wherever the audit runs"* is refuted by a neighbour holding a literal `<…>` filename, which suffix-matches (rare — illegal on NTFS — and real).

### Round 7 — prose only, and it found the consolidation table wrong

Scoped deliberately to prose consistency (the oracle's behaviour is held by rows X1–X20 and ablations A1–A12; re-deriving it costs more than it catches). Eight findings, all in text this release wrote:

- **The table's row for *resolves nowhere, neighbour reachable* said `Findings`.** Measured: the oracle puts it in `Skipped as declared-placeholder` at exit 0, and three other passages in the same step say the same. That row is the correctly-marked placeholder — the case the marker exists for.
- **The unhedged absolute the release claims to have hedged was re-created verbatim in the new paragraph**, one screen above its own hedge: *"excused wherever the audit runs"*, contradicted by the carve-out below it. The named defect of the release, reproduced inside the fix for it.
- **The table had no row for an angle-bracket segment**, while the prose describes two outcomes for one. Added, as the fall-through it actually is.
- `:73`'s *"a marker on a path that does resolve is reported"* — the pre-#56 wording, four lines above the corrected form at `:75`. Two orphaned sentences left by the rewrite (a dangling *"Only once nothing local has answered it"*, and a *"still"* whose antecedent was deleted). *"Split the output into three sections"* while the step now names five. Three CHANGELOG errors, including *"Three were mechanical"* over four bullets.

### Round 6, and what it says about the shape of this document

Round 6 ran on round 5's fix and found nine more defects. Four were mechanical and are fixed here:

- **The new rung-2 arm had no counted-section guard, and the suite was green over the hole.** Replacing its `placeheld.append(...)` with a bare `continue` dropped both of `localmark.md`'s paths from **all six** counted sections and every case still passed — X17/X18 assert `VERDICT: CLEAN`, which a silent drop satisfies perfectly. Every *other* excusing arm in the oracle already had this guard; the identical mutation one arm down is killed instantly by N20. That is the silent-skip failure the fixture exists to prevent, in an arm added by the commit that was fixing a different instance of it. Now X17c, which also pins the collision count inside the message.
- **`review-prompt.md` (N25) started flowing through the new arm and was not in the enumeration list.** Added.
- **Three mutants of the rung-1b message survived green**, including one that reports `resolves at rung 1, as written` for a path that resolved doc-relative — a false provenance, which is the same objection this file makes twice for ordering rung 3 before rung 4. `docs/docrel.md` held one reference, so any finding satisfied `1 finding(s)`. Now X19c asserts the reason, per the fixture's own T19 rule.
- **An angle-bracket segment with a literal file of that name on disk was reported as a `STALE PLACEHOLDER MARKER`** — prescribing the removal of a marker not in the document. The two adjudicating arms now pick the wording from the form that excused the path, seeded as the markerless twin in `docs/docrel.md`.

⚠️ **The other five were all one shape: a rule restated in many places, edited in one.** A stale rung list ("at rung 1 or rung 4") four paragraphs above the new text; "a path a local rung answers stays in the skip section", true for rungs 2 and 3 and false for 1 and 1b; a correction written in the present tense about behaviour the same commit removed; "every rung-2, rung-3 and rung-4 resolution" in the reporting section, widened by the fix and not updated. **Step 4 stated the rung order and its consequences in at least seven separate passages** — the intro limit, the skip-section rule, the local-resolution rule, the qualify-instead advice, the marker/angle-bracket carve-out, the status paragraph and the reporting list — so every release corrected one and left the rest. That is a fair description of rounds 1 through 6 as a whole, not just this one.

**So this release consolidates rather than adds.** One table states, once, what a marked path does at each rung: which rungs *adjudicate*, which only *decide*, and which counted section each outcome lands in. The other passages point at it instead of restating it. ⚠️ **It did not shrink the file: +2164 bytes.** The table costs more than the restatements it replaced, and round 7 then found the table's own fifth row *wrong* — it said `Findings` for a marked path resolving nowhere on a tree that could test it, which is the correctly-marked placeholder, the case the whole feature exists for. An adopter following that row would report every legitimate placeholder as a defect. The suite stayed green because the oracle agreed with the prose and not with the table. What consolidation bought is one place to correct, not fewer bytes; the honest claim is that the *next* contradiction has one home instead of seven.
...
`#35`'s negatives rule and `#39`'s absolutes rule were the same failure from two sides — #39's own issue says so — and they now ship as one instruction rather than two adjacent paragraphs: **a claim that needs a measurement gets one, gets hedged, or is not ready.**

*(An earlier draft of this entry claimed merging "cost less than adding #39 alone". Measured on the lens block: pre-merge 844 bytes, #39 appended separately ≈1,332, merged ≈1,612 — merging cost about 21% **more**. The claim carried no unit and no measurement, in the release that ships the rule against exactly that. Corrected rather than deleted, so the failure stays on the record.)*
...
**And it narrows a separate gap.** Four hypotheses covering this session's load-bearing claims were written retrospectively at the end of it, reconstructing refutation criteria that had been live hours earlier. The cause is narrower than "nothing says when to write one" — `templates/hypothesis-log.md` and `README.md` both say when, in their own how-to-use text. What was missing is a **trigger in the working path**: nothing an agent reads *while making a claim* pointed at the log.

The fix is split across the two artifacts that can each do their half. The lens **reports** an unmeasurable claim as a finding; it cannot register one, since it reports rather than writes and is not placed to supply a Method or a Revisit trigger. `templates/hypothesis-log.md` now says to write the entry at the moment the claim is made, and names the lens finding as the cue.

*(The first draft put registration inside the lens. A review refuted it on four counts: it contradicted the block's own three-option contract two lines above, contradicted "this is not a step to perform", had no write path or output channel in a pre-commit review, and named one of the seven fields the template requires — dropping Method, which that template calls "the whole point". It had been registered an hour earlier as **H-010**, which predicted precisely this: a detector with no actuator.)*
...
**The filter is `sed '/-/d'`, not `grep -v -- '-'`.** ⚠️ **Corrected 2026-08-26 (#91): the measurement below was an artifact of the reviewing agent's own `grep` wrapper, not of this maintainer's machine.** Re-measured on the same shape — `grep -v -- '-'` works correctly under **GNU grep 3.12**, which is what `/usr/bin/grep` actually is here, and under busybox; it prints nothing and exits 1 only under the harness wrapper, which re-execs `ugrep` with injected flags. The conclusion below stands — `sed '/-/d'` is correct on every implementation tried and nothing shipped broken — but the *reason* given for it does not. Left in place rather than rewritten, because the error is the point: the same undetected cause produced a second false claim in v1.28.0, and the pair is what makes it a class rather than a slip. The original text follows.

The first draft used grep and was measured to fail outright on **ugrep**, which is a real `grep` on a real machine — this maintainer's: it parses `-- '-'` as "no pattern specified", exits non-zero, and prints nothing, so the selector silently yields an empty baseline and the reader is sent down the first-release path. Caught by running the command rather than reading it, in the same session that wrote the paragraph telling adopters to run it. This is the same "hostile environment, not the clean one" rule the project file already carries, arriving from an angle it did not name: not a hostile *repo*, a hostile `PATH`.
