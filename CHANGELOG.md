...
Rule 2's silence is the trap rules 6 and 12 are both built around — an absent population and a clean one producing byte-identical output — sitting two lines above them, in the rule that runs first. Rule 2 now reports a **SKIP** and counts it; rule 1 exempts a `memory/` path only when **the whole directory is absent** — the fresh-clone shape — and **prints how many it exempted**. CI's own run: `27 file and 17 directory reference(s) checked; 4 + 1 exempt` and `2 rule(s) SKIPPED — a skipped rule is not a passed rule`, with the skips surfaced to the run summary rather than folded into the pass.
