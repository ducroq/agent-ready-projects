...
**Proposed bump when it does go out: MINOR** — a new lint rule and new behaviour on **four** normative surfaces (`git diff v1.36.1 -- templates/`: `audit-context`, `curate`, `release`, `review-changes`). This line said *two* while three had already changed, and the count is checkable, so it is now stated as a command.
