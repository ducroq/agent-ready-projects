...
For Claude Code: `curate`, `audit-context` and `review-changes` are USER-GLOBAL skills — install them once at ~/.claude/skills/<name>/SKILL.md, NOT inside this repo. Copy the ready-made files, which already carry valid frontmatter, from a local clone at agent-ready-projects/.claude/skills/<name>/SKILL.md or from https://raw.githubusercontent.com/ducroq/agent-ready-projects/master/.claude/skills/curate/SKILL.md and https://raw.githubusercontent.com/ducroq/agent-ready-projects/master/.claude/skills/audit-context/SKILL.md. Do NOT copy templates/curate.md or templates/audit-context.md into a SKILL.md path: those files carry their frontmatter INSIDE a `<!-- SAVE AS: ... -->` comment, and a copy that leaves it there has no frontmatter at all, so the skill never registers — it fails by doing nothing, which is why this mistake survives for months. If ~/.claude/skills/<name>/ already exists, leave it alone and do not create a project-local copy: a user-global skill SHADOWS a project-local one of the same name, so the local copy would be inert while looking authoritative.
Then update the project file's "Before You Start" table to include: "Ending a session → Run /curate" and "Monthly or after major restructuring → Run /audit-context". The skills are global; the pointers are per-repo.
