...
# N2 — a link to a DIRECTORY named *.sh is not a script (review finding).
mkdir -p "$WORK/n2/tests" "$WORK/n2/realdir"; printf 'true\n' > "$WORK/n2/tests/ok.sh"
ln -s ../realdir "$WORK/n2/tests/dirlink.sh"
want_clean "N2 a symlink to a directory is skipped" "$WORK/n2"

# T6 — a dangling link is reported as dangling, not as a parse failure.
mkdir -p "$WORK/t6/tests"; printf 'true\n' > "$WORK/t6/tests/ok.sh"
ln -s /nonexistent/target.sh "$WORK/t6/tests/dangling.sh"
want_fail "T6 a dangling symlink is reported as dangling" "$WORK/t6" "tests/dangling.sh: dangling symlink"
...
ablate "A3 skip symlinks"            '\( -type f -o -type l \)' '-type f' "t5,t6"
ablate "A4 dangling links pass"       'bad=$((bad + 1)); printf' ': ; printf' "t6"
