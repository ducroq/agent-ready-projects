...
**Case counts, re-measured 2026-09-24** (`grep -oE '\bT[0-9]+[a-z]?\b' run.sh | sort -u | wc -l`, and the same for `N`): **62 T-cases** and **60 N-cases**; `grep -c '^ablate "' run.sh` gives **12 ablations**; `grep -cE '^  "X[0-9]+ ' run.sh` gives **18 X rows**. ⚠️ **This line said 27 and 28 from 2026-09-03 until 2026-09-12, with the re-deriving command sitting beside the stale digits the whole time.** That is why the check proposed in `memory/gotcha-log.md` is *does the stated number equal what the command returns*, not *is a command present* — the weaker predicate passes this very file. #102 added T28, T29 and N32; #175 added T48-T51 and N43-N46 (one pair per module extension, because `EXT` is an alternation and one case cannot vouch for its neighbours); #165/#176 added T52-T54 and N47; #177 added T56; #122 added T57-T61 and N50; #165 added N51 and N52 (T18's class for the second member COMPOUND_EXT adds, which an adversarial lens found uncovered); #199 added T62 and N53 (one pair per documents-repo extension, generated from one list in each of build.sh and run.sh), T63-T65, N54, N55 and N56; #155 added N57-N61 (one per spelling of negated existence) T66, the positive-probe control, and T67, a quoted negation that must not excuse a real reference; #154 added T68 and N62-N66 (resolutions inside a gitignored directory are listed, never ruled). ⚠️ #154's T55/N48/N49 were added and then REVERTED in the same session — the change they seeded contradicted SPEC.md's rung-2 rule and produced 60 false positives on this repo; the digits above were stale for the hour in between, which is this paragraph's own subject. The numbering is not contiguous and cannot be written as a range — there is no T6, T20, N4, N5 or N29–N31 — so quote the command, never a span. ⚠️ **T28 and T29 are needled on the PATH, not the reason**, which T21's rule normally forbids: the oracle prints `UNRESOLVED` both when the gate declines a reference and when the target is absent, so no reason string separates them, and a needle carrying the reason is keyed on column padding that moves with the path length. The discriminator is an existence check on the two target files instead. The enumeration below is the original eleven and is kept for the reasoning, not as a census; treat any number in prose here as dated unless it carries a command.
