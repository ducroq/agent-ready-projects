...
⚠️ **Rules 10, 12 and 13 write a coverage note to stderr and the caller must assert it appeared.** A checker that could not run — no name list, no ablations found — produces exactly the same empty stdout as a clean one. Reading empty stdout as "clean" is the failure these two rules exist to avoid, so the runner escalates a missing coverage line rather than folding it into a pass.
