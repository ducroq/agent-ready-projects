...
| **10. No ablation that cannot kill anything** | An ablation is added, reviewed and merged that **cannot fail** — the mutated check behaves identically, so the suite reports a passing guard that tests nothing. #79's shape. ⚠️ **Scope is deliberately narrow**, because "a check that cannot fail" is undecidable in general: two lexical shapes only, both observed here — a mutation whose replacement equals its target modulo whitespace, and one applied to text that does not occur. Implemented in `vacuous-guard.sh`; fixture co-seeds a control each mutant must keep passing | `FAIL  ablation replaces a string with itself — cannot kill anything` |
| **11. Fenced bash blocks on adopter surfaces parse** | A block an adopter copies is a shell syntax error. #105: an ASCII apostrophe inside a single-quoted awk program (`Step 1.5's property`) closed the program, and the backticked regex below became command substitution. **It shipped for eight releases** and nothing here could see it — rule 6 compared two copies carrying the same apostrophe, so they agreed perfectly; the ratchet measures bytes; and every internal run transcribed the awk into a fresh heredoc instead of copying the block. Exemptions are **declared, not guessed**. Implemented in `block-parses.sh`; fixture at `tests/fixtures/block-parses/` — ⚠️ **it has no ablations**: `grep -c ablat` returns 0, and this row claimed two until 2026-09-14. Its seeded cases show the check firing; nothing shows a mutant of it going quiet | ``FAIL  templates/review-changes.md block 3: syntax error near unexpected token `rest,'`` |
| **12. No private project name in a tracked file** | This repo is public and its author works in an estate of private ones. Names reached shipped `templates/`, released `CHANGELOG.md` entries and four fixtures — 42 occurrences across 10 files — before this rule existed. ⚠️ **The name list is NOT tracked**, because a tracked denylist publishes exactly what it protects: it lives at `~/.config/agent-ready/private-names` (or `$AGENT_READY_PRIVATE_NAMES`), and **an absent list is a reported SKIP, never a pass**. Short or generic names are declared UNCHECKED rather than matched. Population is tracked **plus untracked-not-ignored**, so a new file is covered before it is committed. Implemented in `private-names.sh`; fixture at `tests/fixtures/private-names/` (its own list is synthetic) | `FAIL  templates/curate.md:88: private project name` |
| **13. No maintainer-only path on an adopter-installed surface** | A skill body points at `docs/rationale/` — a directory that holds *this* repo's litigation about its own skills and that no template tells an adopter to create, so the pointer is dead on every install. v1.37.0 shipped five; v1.38.0 removed all five; a change hours later re-created one in the same two files and cost a full review round to find (#139, promoted per #127). ⚠️ **The denylist is one entry on purpose** — `CHANGELOG.md` (17 hits), `tests/lint/run.sh` and `tests/fixtures/` are all legitimate there, measured. Exemption is declared (`lint-skip: maintainer-path`), never guessed — **and now validated**: the skip is granted only when the matched path sits inside an `https://` run, because a repo-relative path is dead in every adopter clone, which is the defect the rule exists to catch. Measured: the bare marker previously passed one |
| **14. Step 1.5 finds nothing in this repo's own markdown** | A corruptible markdown shape lands and stays. ⚠️ **Not a regression check on Step 1.5 — the check already worked and was not being RUN.** Audited 2026-09-14: `.claude/review-profile.md` carried a two-glob bold span through four consecutive commits that touched it (`eea6530`..`1c443eb`, measured by running the shipped program against each revision), while the skill claims to run on every changed markdown file at every tier. It also reaches what the skill cannot: every term of Step 1.5's own file list is git-sourced, so the gitignored markdown under `memory/` — the layer whose wide tables are its stated motivation — is invisible to it, and the gitignored share is printed on every run rather than recorded here, because it moves with every file added. The program is **extracted, never copied** (`extract-step15.py`, shared with the fixture). Exemptions are content-keyed, never line-numbered, and bounded three ways: one path, the source line **and the finding's shape**, and **exactly one match** — zero is STALE, more than one is OVER-BROAD, and both FAIL. ⚠️ The shape bound exists because a review measured the path+substring form transferring an emphasis exemption onto a lossy row at exit 0. Implemented in `step15-corpus.sh`; fixture at `tests/fixtures/step15-corpus/` | `FAIL  ./memory/MEMORY.md:28: row has 17 cells, table defines 3 — the excess is dropped when rendered` |
| **15. The auto-loaded set only ratchets down** | `CLAUDE.md` and `memory/MEMORY.md` grow, and nothing charges for it — the one cost paid **every session, before anyone asks for anything**. Measured at 49,172 bytes on 2026-09-14 against the 40,000 that `audit-context` flags, and it grew during the session that was cutting it. ⚠️ **No threshold is invented here**: 40,000 is where Claude Code *warns*, and the same skill says no character count for the index has ever been derived — so both numbers are ratchets, like rule 8. Two of them: total bytes, and the index's **line** count, because an index is a list. An unlocked shrink is reported too — a reduction nobody locks in leaves the ceiling where it was and drifts straight back. **SKIPS when `memory/` is absent** (CI, fresh clone): measuring half the set would read as a huge shrink. Implemented in `autoload-ratchet.sh`; baseline `autoload-baseline.tsv`; fixture at `tests/fixtures/autoload-ratchet/` | `FAIL  auto-loaded set GREW: 51204 bytes against 49172 (+2032)` |
| **16. A skill template's SAVE AS agrees with how it is installed** | A template copied verbatim to a `SKILL.md` path carries its frontmatter **inside a comment**, so the skill registers as nothing — it **fails by doing nothing** (#184). It took three review rounds to stop finding instances: each round fixed what it was shown and none asked how many there were; the census that ended it is one line. ⚠️ **No other rule reaches the class**: rule 3 checks only that `name:`/`description:` exist, rule 6 excludes the SAVE AS block by construction — so both copies agree while contradicting five other surfaces. Two halves: the do-not-copy-verbatim clause is present, and the stated scope agrees with `GLOBAL_SKILLS` in `scripts/install-global-skills.sh`, **derived and never listed** (a second list is the drift this catches, one file over). ⚠️ **It binds the template header and nothing else** — the instruction that originated #184 lived in `docs/GUIDE.md`, and that file and `templates/README.md` each carry a further hand-maintained copy of the same scope fact; flipping the scope in both scores 0 here, measured. Three of the five surfaces the gotcha row names are still unbound. Implemented in `save-as-header.sh`; fixture at `tests/fixtures/save-as-header/` | `FAIL  templates/foo.md:3: foo is in GLOBAL_SKILLS but its SAVE AS path is not \`~/.claude/skills/foo/\`` |
| **17. No shell option welded to a comment** | `#` opens a comment only at a **word start**, so `set -u# nounset` passes `-u#` as an option, errors at rc 2, and **never applies the option** — the script runs on with the guard it appears to have. ⚠️ **`bash -n` passes on it**, measured, so #160's proposed parse sweep does not cover this; they are complementary. Found originally by a review lens with five instruments green (runners exited 0, `bash -n` passed, lint was 13/13, the fixture runner swallowed the stderr, the author's "all PASS" was true) — the most expensive way there is to find a typo. Keyed on the character before the `#`, not on the option run: a first draft keyed on the run and missed `set -eo pipefail# x`. ⚠️ **What it cannot see**: the class is *a `#` that is not at a word start*, and the predicate is `set` at line start — so `shopt -s nullglob# x`, `export LC_ALL=C# x`, `trap … EXIT# x` and any non-line-initial `set` are all missed, each measured and each passing `bash -n`. ⚠️ **And the consequence varies**: `set -u#` is silent and continues, `set -eo pipefail#` applies `-e` and aborts loudly at rc 2 — a draft of this rule's message claimed the option is never applied and the script runs on, for all of them. Exemption is declared, never guessed, and a marker on a line with no defect is reported STALE — the unused-suppression property the prior art carries and the first draft lacked. ⚠️ **The marker is honoured only where it is policed** — `*.sh`/`*.bash`, less this rule and its own fixture. That is one predicate, not two: an earlier build honoured `lint-skip: opt-comment` everywhere while policing only shell, so a marker in a markdown fenced block was never checked for staleness **and still suppressed the real defect on its line** — in `templates/*.md`, the surface adopters copy. Where it is not policed it is not honoured, so a demonstration in markdown is reported: noise, not silence. The scan fired on four self-reference sites before this settled, which is #174's axis — a marker cannot be written about in its own syntax. Implemented in `opt-comment.sh`; fixture at `tests/fixtures/opt-comment/` | ``FAIL  ./tests/foo.sh:3: a shell option welded to a comment … the option is NEVER APPLIED`` |


⚠️ **Most rules write a coverage note to stderr and the caller asserts it appeared** — 6, 7, 8, 9, 10, 11 and 12-17 do. A checker that could not run — no name list, no ablations found, an extraction whose anchors moved — produces exactly the same empty stdout as a clean one. Reading empty stdout as "clean" is the failure they exist to avoid, so the runner escalates a missing coverage line rather than folding it into a pass.
...
- **Anything rule 14's exemptions cover.** One exemption ships today: prose in `CHANGELOG.md` that quotes the corrupt shape inside a double-backtick span, which the checker reads as two single-backtick ones (#159). While it matches, that one line is unchecked *for that one shape* — a different shape on the same line still reports. ⚠️ **And it is content-scoped, not line-scoped**, which is the axis on which it differs from the `# noqa` / `# shellcheck disable` prior art it otherwise imitates: those cannot transfer because the host syntax binds them to a line, and this one is bounded by a cardinality check instead.
- **Rule 14 under CI.** `memory/` does not exist in a fresh clone, so the gitignored half of the population is empty and the rule is weaker there — 92 files against 106 locally, measured. It does **not** SKIP (it really did scan what it found), so it does not appear in the runner's SKIPPED tally; the coverage line says `(none here — this run is WEAKER …)` and that line is the only tell.
- **A seeded defect anywhere in the tree.** The population is every markdown file, so a future fixture README or doc that *demonstrates* a corrupt shape will fail rule 14, and the only escape is an exemption. That is a real cost of scanning the whole tree, accepted deliberately: the alternative is an ignore list that grows silently.
- **The one-token emphasis form.** Step 1.5 reports only two `**`-suffixed globs inside one bolded phrase; the single-glob form breaks under prettier 2 and 3.8.1 and passes here in silence (#151, precision tracked as #158). Rule 14 inherits that blind spot exactly — it runs the shipped program, so it cannot be stronger than it.
