...
echo "[1/10] CLAUDE.md path references resolve on disk"
...
echo "[2/10] memory/MEMORY.md index integrity"
...
echo "[3/10] skill template embedded SKILL.md frontmatter"
...
echo "[4/10] installed skills are loadable"
...
echo "[5/10] top-level YAML frontmatter closure"
...
echo "[6/10] skill templates and reference installs agree"
...
echo "[7/10] a skill that provisions a canonical row must quote it"
...
echo "[8/10] adopter-facing templates have not grown"
...
echo '[9/10] no bare $0-$9 in a skill body'
...
echo '[10/10] no ablation that cannot kill anything'
