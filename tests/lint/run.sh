...
echo "[1/20] CLAUDE.md path references resolve on disk"
...
echo "[2/20] memory/MEMORY.md index integrity"
...
echo "[3/20] skill template embedded SKILL.md frontmatter"
...
echo "[4/20] installed skills are loadable"
...
echo "[5/20] top-level YAML frontmatter closure"
...
echo "[6/20] skill templates and reference installs agree"
...
echo "[7/20] a skill that provisions a canonical row must quote it"
...
echo "[8/20] adopter-facing templates have not grown"
...
echo '[9/20] no bare $0-$9 in a skill body'
...
echo '[10/20] no ablation that cannot kill anything'
...
echo '[11/20] no fenced bash block that an adopter copies fails to parse'
...
echo "[12/20] no private project name in a tracked file"
...
echo "[13/20] no maintainer-only path referenced from an adopter-installed surface"
...
echo "[14/20] review-changes Step 1.5 finds nothing in this repo's own markdown"
...
echo "[15/20] the auto-loaded set only ratchets down"
...
echo "[16/20] a skill template's SAVE AS agrees with how the skill is installed"
...
echo "[17/20] no shell option welded to a comment"
...
echo "[18/20] every release tag has a dated CHANGELOG.md block"
...
echo "[19/20] a release that grows the adopter-facing surface says so"
...
echo "[20/20] every shell file under tests/ and scripts/ parses"
