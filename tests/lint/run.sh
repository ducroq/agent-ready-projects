...
echo "[1/15] CLAUDE.md path references resolve on disk"
...
echo "[2/15] memory/MEMORY.md index integrity"
...
echo "[3/15] skill template embedded SKILL.md frontmatter"
...
echo "[4/15] installed skills are loadable"
...
echo "[5/15] top-level YAML frontmatter closure"
...
echo "[6/15] skill templates and reference installs agree"
...
echo "[7/15] a skill that provisions a canonical row must quote it"
...
echo "[8/15] adopter-facing templates have not grown"
...
echo '[9/15] no bare $0-$9 in a skill body'
...
echo '[10/15] no ablation that cannot kill anything'
...
echo '[11/15] no fenced bash block that an adopter copies fails to parse'
...
echo "[12/15] no private project name in a tracked file"
...
echo "[13/15] no maintainer-only path referenced from an adopter-installed surface"
...
echo "[14/15] review-changes Step 1.5 finds nothing in this repo's own markdown"
...
echo "[15/15] the auto-loaded set only ratchets down"
