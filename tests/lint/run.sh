...
echo "[1/23] CLAUDE.md path references resolve on disk"
...
echo "[2/23] memory/MEMORY.md index integrity"
...
echo "[3/23] skill template embedded SKILL.md frontmatter"
...
echo "[4/23] installed skills are loadable"
...
echo "[5/23] top-level YAML frontmatter closure"
...
echo "[6/23] skill templates and reference installs agree"
...
echo "[7/23] a skill that provisions a canonical row must quote it"
...
echo "[8/23] adopter-facing templates have not grown"
...
echo '[9/23] no bare $0-$9 in a skill body'
...
echo '[10/23] no ablation that cannot kill anything'
...
echo '[11/23] no fenced bash block that an adopter copies fails to parse'
...
echo "[12/23] no private project name in a tracked file"
...
echo "[13/23] no maintainer-only path referenced from an adopter-installed surface"
...
echo "[14/23] review-changes Step 1.5 finds nothing in this repo's own markdown"
# Rule 14 — audited 2026-09-14. The check already shipped and already worked;
# what it was not doing is RUNNING. `.claude/review-profile.md` carried a
# corruptible two-glob bold span through four consecutive commits that touched
# it (`eea6530`..`1c443eb`, measured per revision), while Step 1.5 claims to run
# on every changed markdown file at every tier.
# ⚠️ It also reaches what the skill cannot: every term of Step 1.5's own file
# list is git-sourced, so the gitignored markdown under memory/ is invisible to
# it — the layer whose wide tables are the step's stated motivation. In CI that
# half of the population is empty, so a green CI run is WEAKER than a green
# local one; the coverage line reports the two counts separately.
# Exemptions are content-keyed, counted, and reported, and a stale one FAILS —
# see the checker. Fixture at tests/fixtures/step15-corpus/.
s15_out=$(mktemp); s15_err=$(mktemp)
# `env -u`: $STEP15_EXEMPTIONS exists so the FIXTURE can seed a run with no
# exemptions, and nothing else scoped it — exporting it to a file of one throwaway
# entry made this rule pass with the shipped allowlist wholly replaced, at rc 0.
# Rule 12's env var can only SUPPLY a population; this one could subtract every
# check, which is the "blanket skip" the runner's guarantee forbids.
env -u STEP15_EXEMPTIONS bash tests/lint/step15-corpus.sh . >"$s15_out" 2>"$s15_err"; s15_rc=$?
cat "$s15_err"
if [ $s15_rc -gt 1 ]; then
  fail "rule 14 checker could not run (exit $s15_rc) — this rule scanned nothing"
elif ! grep -q 'markdown file(s) found' "$s15_err"; then
  fail "rule 14 checker produced no coverage line — this rule scanned nothing"
else
  while IFS= read -r line; do
    [ -n "$line" ] && fail "$line"
  done < "$s15_out"
...
# threshold invented here either — both numbers are ratchets.
# ⚠️ memory/MEMORY.md is gitignored, so CI has half the population. The checker
# SKIPS rather than measuring the remaining half, which would read as a large
# shrink and fail the lock-it-in arm. A skip is counted, never folded into a pass.
al_out=$(mktemp); al_err=$(mktemp)
bash tests/lint/autoload-ratchet.sh . >"$al_out" 2>"$al_err"; al_rc=$?
cat "$al_err"
if [ $al_rc -gt 1 ]; then
  fail "rule 15 checker could not run (exit $al_rc) — this rule measured nothing"
elif grep -q '^SKIPPED:' "$al_err"; then
  SKIPPED=$((SKIPPED + 1))
elif ! grep -q 'auto-loaded file(s) measured' "$al_err"; then
  fail "rule 15 checker produced no coverage line — this rule measured nothing"
else
  while IFS= read -r line; do
    [ -n "$line" ] && fail "$line"
  done < "$al_out"
...
echo "[16/23] a skill template's SAVE AS agrees with how the skill is installed"
# Rule 16 — #184, promoted from a review finding per #127. A template copied
# verbatim to a SKILL.md path carries its frontmatter inside a comment, so the
# skill REGISTERS AS NOTHING — it fails by doing nothing. Three review rounds were
# needed to stop finding instances. ⚠️ No other rule reaches it: rule 3 checks only
# that the keys exist, rule 6 excludes the SAVE AS block by construction, so both
# copies agree while contradicting five other surfaces. The scope half is DERIVED
# from GLOBAL_SKILLS in the installer, never listed here.
# Fixture at tests/fixtures/save-as-header/.
sa_out=$(mktemp); sa_err=$(mktemp)
bash tests/lint/save-as-header.sh . >"$sa_out" 2>"$sa_err"; sa_rc=$?
cat "$sa_err"
if [ $sa_rc -gt 1 ]; then
  fail "rule 16 checker could not run (exit $sa_rc) — this rule checked nothing"
elif ! grep -q 'skill template(s) checked' "$sa_err"; then
  fail "rule 16 checker produced no coverage line — this rule checked nothing"
else
  while IFS= read -r line; do [ -n "$line" ] && fail "$line"; done < "$sa_out"
fi
rm -f "$sa_out" "$sa_err"

echo "[17/23] no shell option welded to a comment"
# Rule 17 — #160, promoted from a review finding. `#` opens a comment only at a
# word start, so `set -u# why` passes `-u#` as an option, errors at rc 2, and
# NEVER APPLIES the option. ⚠️ `bash -n` PASSES on it — measured — so #160's
# proposed bash -n sweep does not cover this; they are complementary. Found
# originally by a review lens with five instruments green, which is the most
# expensive way there is to find a typo. Fixture at tests/fixtures/opt-comment/.
oc_out=$(mktemp); oc_err=$(mktemp)
bash tests/lint/opt-comment.sh . >"$oc_out" 2>"$oc_err"; oc_rc=$?
cat "$oc_err"
if [ $oc_rc -gt 1 ]; then
  fail "rule 17 checker could not run (exit $oc_rc) — this rule scanned nothing"
elif ! grep -q 'file(s) found' "$oc_err"; then
  fail "rule 17 checker produced no coverage line — this rule scanned nothing"
else
  while IFS= read -r line; do [ -n "$line" ] && fail "$line"; done < "$oc_out"
fi
rm -f "$oc_out" "$oc_err"

echo "[18/23] every release tag has a dated CHANGELOG.md block"
# Rule 18 — #197. v1.45.1 was tagged and pushed with its block still reading
# "(candidate, unreleased)"; three checks agreed it was released because each read
# the tag, none the heading adopters read. SKIPS where no release tag exists (CI
# fetches none). Fixture at tests/fixtures/released-heading/.
rh_out=$(mktemp); rh_err=$(mktemp)
bash tests/lint/released-heading.sh . >"$rh_out" 2>"$rh_err"; rh_rc=$?
cat "$rh_err"
if [ $rh_rc -eq 3 ]; then
  SKIPPED=$((SKIPPED + 1))
elif [ $rh_rc -gt 1 ]; then
  fail "rule 18 checker could not run (exit $rh_rc) — this rule checked nothing"
elif ! grep -q 'release tag(s) checked' "$rh_err"; then
  fail "rule 18 checker produced no coverage line — this rule checked nothing"
else
  while IFS= read -r line; do [ -n "$line" ] && fail "$line"; done < "$rh_out"
fi
rm -f "$rh_out" "$rh_err"

echo "[19/23] a release that grows the adopter-facing surface says so"
# Rule 19 — H-022. Rule 8's budget can always be raised, so it never limited
# growth. When the top CHANGELOG.md block is dated, templates/ is compared with
# the previous release tag, and growth must be stated in that block as
# "Adopter-facing size: +N bytes". SKIPS where no release tag exists (CI).
# Fixture at tests/fixtures/release-growth/.
rg_out=$(mktemp); rg_err=$(mktemp)
bash tests/lint/release-growth.sh . >"$rg_out" 2>"$rg_err"; rg_rc=$?
cat "$rg_err"
if [ $rg_rc -eq 3 ]; then
  SKIPPED=$((SKIPPED + 1))
elif [ $rg_rc -gt 1 ]; then
  fail "rule 19 checker could not run (exit $rg_rc) — this rule checked nothing"
elif ! grep -qE 'release-growth: (top block|v[0-9])' "$rg_err"; then
  fail "rule 19 checker produced no coverage line — this rule checked nothing"
else
  while IFS= read -r line; do [ -n "$line" ] && fail "$line"; done < "$rg_out"
fi
rm -f "$rg_out" "$rg_err"

echo "[20/23] every shell file under tests/ and scripts/ parses"
# Rule 20 — #160. Rule 11 parses the fenced bash adopters copy; nothing parsed the
# fixtures and scripts, where two of #160's four quoting breaks landed. A fixture
# that does not parse still runs every line above the break and reads like a
# sensitivity failure. Parses only; see the checker header for what that misses.
# Fixture at tests/fixtures/shell-parse/.
sp_out=$(mktemp); sp_err=$(mktemp)
bash tests/lint/shell-parse.sh . >"$sp_out" 2>"$sp_err"; sp_rc=$?
cat "$sp_err"
if [ $sp_rc -gt 1 ]; then
  fail "rule 20 checker could not run (exit $sp_rc) — this rule checked nothing"
elif ! grep -q 'file(s) found' "$sp_err"; then
  fail "rule 20 checker produced no coverage line — this rule checked nothing"
else
  while IFS= read -r line; do [ -n "$line" ] && fail "$line"; done < "$sp_out"
fi
rm -f "$sp_out" "$sp_err"

echo "[21/23] scaffolding templates carry the release they ship in"
# Rule 21 — #171. The v1.41.0 tag shipped templates stamped v1.40.0, and
# templates/release.md Step 5 had named that class in prose. Checks the stamps
# against a dated top CHANGELOG.md block (runs in CI), else the highest reachable
# tag, and the tag's own templates via git show. SKIPS with a candidate block and
# no tags. Fixture at tests/fixtures/template-stamp/.
ts_out=$(mktemp); ts_err=$(mktemp)
bash tests/lint/template-stamp.sh . >"$ts_out" 2>"$ts_err"; ts_rc=$?
cat "$ts_err"
if [ $ts_rc -eq 3 ]; then
  SKIPPED=$((SKIPPED + 1))
elif [ $ts_rc -gt 1 ]; then
  fail "rule 21 checker could not run (exit $ts_rc) — this rule checked nothing"
elif ! grep -q 'checked against' "$ts_err"; then
  fail "rule 21 checker produced no coverage line — this rule checked nothing"
else
  while IFS= read -r line; do [ -n "$line" ] && fail "$line"; done < "$ts_out"
fi
rm -f "$ts_out" "$ts_err"

echo "[22/23] install paths on GUIDE.md, templates/README.md and adopt.md agree with the installer"
# Rule 22 — #195. Rule 16 binds each template's SAVE AS header; these three
# surfaces carry further hand-maintained copies of the same scope fact, and #184
# began in GUIDE.md. Reads prefixed install paths only; see the checker header.
# Fixture at tests/fixtures/scope-surfaces/.
ss_out=$(mktemp); ss_err=$(mktemp)
bash tests/lint/scope-surfaces.sh . >"$ss_out" 2>"$ss_err"; ss_rc=$?
cat "$ss_err"
if [ $ss_rc -gt 1 ]; then
  fail "rule 22 checker could not run (exit $ss_rc) — this rule checked nothing"
elif ! grep -q 'install path(s) found' "$ss_err"; then
  fail "rule 22 checker produced no coverage line — this rule checked nothing"
else
  while IFS= read -r line; do [ -n "$line" ] && fail "$line"; done < "$ss_out"
fi
rm -f "$ss_out" "$ss_err"

echo "[23/23] a verification rating is the floor of its steps"
# Rule 23 — #168. A source whose step table records PARTIAL, NEEDS WORK or FAIL
# was rated VERIFIED, and its claims ESTABLISHED, for five months. Reads
# docs/vv/verification-log.md and joins docs/vv/claims/claim_registry.md by claim
# ID. Fixture at tests/fixtures/rating-floor/.
rf_out=$(mktemp); rf_err=$(mktemp)
bash tests/lint/rating-floor.sh . >"$rf_out" 2>"$rf_err"; rf_rc=$?
cat "$rf_err"
if [ $rf_rc -gt 1 ]; then
  fail "rule 23 checker could not run (exit $rf_rc) — this rule checked nothing"
elif ! grep -q 'source block(s)' "$rf_err"; then
  fail "rule 23 checker produced no coverage line — this rule checked nothing"
else
  while IFS= read -r line; do [ -n "$line" ] && fail "$line"; done < "$rf_out"
fi
rm -f "$rf_out" "$rf_err"
