...
- **`<!-- placeholder -->` on the line**, mirroring the `<!-- verify: -->` idiom — invisible when rendered, greppable. It is **line-scoped**, so it must sit on the same line as the path it marks; a marker on a line carrying no extractable path is reported as an **ineffective marker**, because a marker that silently does nothing is the failure this whole step is built against.
