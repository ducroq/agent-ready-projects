...
⚠️ **An entry that is filename-shaped rather than extension-shaped re-admits the phantom class the whitelist exists to exclude.** The rule matches the tail of any dotted token, so whitelisting `env` captures `process.env` — a ubiquitous code identifier that no rung can ever resolve. Keep such a token only when it still looks like a path: it contains a `/` (`config/settings.env`), or it starts with a `.`. Measured on `env`; `example`, `gitignore` and `dockerfile` are the same shape and are left alone until a collision is actually observed, because tightening on argument rather than evidence loses sensitivity nobody notices.
