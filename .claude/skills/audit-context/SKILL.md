...
⚠️ **"Does resolve" means at *any* rung, including rung 4.** Test the marked path against every reachable sibling repo, not only the ones named in prose — a marked reference typically does not name its repo, which is exactly why the normal rung-4 gate cannot fire on it. Cross-repo paths are the population most likely to be marked in the first place, so a local-only test is blind precisely where the convention gets used most, and the excused path leaves the checked set **permanently**: if the sibling file later moves or is deleted, nothing reports it. Seeded as T19, with N17 holding the other direction (a path absent everywhere stays excused). Marked paths are few, so the unconditional walk is bounded.

**Before marking a cross-repo path, try qualifying it instead** — name the repo in the surrounding prose so rung 4 resolves it. A qualified reference is checked on every run; a marked one is never checked again. Same reasoning this step already gives for preferring qualification over suppression.
