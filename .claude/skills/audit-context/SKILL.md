...
| nowhere, a neighbour reachable | a rung that ran ruled it out | Findings |
| nowhere, no neighbour reachable | undecided — rung 4 could not run | Unconfirmed |

An **angle-bracket segment** is decided by its shape before any of this, so it is excused wherever the audit runs; a path carrying both forms follows the shape. Two drafts got that ordering wrong in the two available directions.
...
⚠️ **A `<!-- placeholder -->` marker is rung-4 traffic too — and an angle-bracket segment is not.** The stale-marker test runs a marked path against the neighbours, so where none is reachable that test cannot run and such a reference is *undecided*, not excused; excusing it silently returns *clean* on a repo where a reachable neighbour would have reported a stale marker. Only once nothing local has answered it — the table above is the order. **An angle-bracket segment is decided by its shape instead**, so rung 4 declines it on every tree measured and it stays excused; treating it as undecided moves a repo whose placeholders are all of that form from *clean* to *coverage incomplete* on a fresh clone, which is this step's own defect one bucket over. ⚠️ **"Shape" does not mean nothing on disk can touch it.** A literal file of that name — `docs/<slug>.md` on disk, next to the document naming it — resolves at rung 1 or 1b and is reported, with a wording that does **not** say *stale marker*, since there is no marker to remove. A *neighbour* holding such a file suffix-matches at rung 4 the same way. Both are rare (the name is illegal on NTFS), both measured.
