...
- **Any non-frontmatter edit under `.claude/skills/**`** — HIGH because a defect there ships to every install derived from it; that is as true of a three-line body edit as of a frontmatter one. *(Keep this to one glob. A second `**`-suffixed glob in the same bolded phrase is rewritten by prettier 2 — `` `a/**`or`b/**` ``, spaces eaten — because the glob's trailing `**` is parsed as a bold delimiter. Measured: breaks on prettier 2, survives prettier 3, and the single-glob form survives both. It is correct in the diff and wrong only when rendered, and Step 1.5 does not catch it — it checks tables and fences, not emphasis. Put extra paths in a parenthetical instead.)*
