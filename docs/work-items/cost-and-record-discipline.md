...
**Consequence, from the skill's own rule.** `curate` Step 1.3 flags *any unresolved entry older than 14 days*, excusing only what the Promoted table records. Promoted holds **5 data rows**. So ~67 entries here qualify as lingering on **every** `/curate` run, forever — and Step 1.3 already documents that failure mode against a *different* log ("in one measured log 11 entries are recorded resolved in the table and carry no marker in their header, so a header-only pass reports every one of them as lingering on every run, forever") while this repo runs it at six times that scale. We shipped the diagnosis and did not apply it to ourselves.
