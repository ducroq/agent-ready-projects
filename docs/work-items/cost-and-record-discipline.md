...
- [ ] **Estate cleanup (#169)** — ⚠️ **re-measured 2026-09-14 after the v1.42.0 refresh: THREE inert copies, not 13** (`_meta/disentangled-infrastructure`, `FyE/core`, `agent-ready-research`), and the bare `--check` tail reports 12 issues total, which is NOT the same number. Check each for local modifications before deleting; keep every `.claude/review-profile.md`.
