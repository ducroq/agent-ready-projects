...
### A second external result — and this one measures the mechanism, not the premise

*WikiSkill: Compiling Agent Experience into Persistent Knowledge for Skill Evolution* ([arXiv:2608.27454](https://arxiv.org/abs/2608.27454), Google Research and Virginia Tech, August 2026) separates an agent's workspace into three layers — immutable execution traces, a **persistent knowledge base that compounds and is never reset**, and an evolving set of executable skills — and evolves the skills against the accumulated knowledge across iterations. That is this method's shape arrived at independently: raw session history, an accumulation layer, and versioned procedures.

Their ablation is the part worth citing. Holding everything else fixed and removing the accumulation layer — no consolidation of traces into persistent knowledge between iterations — average benchmark performance falls from **63.7% to 48.7%**, across five benchmarks on Gemini-3.5-Flash. Skill quality also kept improving into later iterations rather than saturating early: 39–58% of accepted skill updates land after the first stage.

**What it supports, and what it does not.** It is the first external evidence that *an accumulation layer improves work done later* — the assumption this whole framework rests on and has never been able to measure. It is **not** evidence for anything this repo ships. The population is LLM agents executing benchmark tasks, the outcome is a validation score on held-out tasks, and the loop is machine-driven with automatic gating and rollback. The open question here (`memory/hypothesis-log.md`, H-017) is whether a *human-maintained* repo's accumulated memory is reached for by later sessions at all, in repos this author does not maintain. **That question is untouched by this result**, and a reader who cites this paper for it has substituted a different population and a different outcome measure for the one in question. Cite it for the mechanism; do not cite it for the adoption.

⚠️ **One of their findings runs against this framework's current routing, and is recorded here because it is uncomfortable rather than because it is convenient.** Giving the *executing* agent access to the accumulated knowledge — not just the skill — **degraded** final skill quality, 63.7% to 60.9%. Their reading: the agent solves from the accumulated notes instead of from the skill, so the trajectory stops revealing what the skill lacks. This framework routes working sessions into the gotcha log by design. Whether that trade is wrong here is an open hypothesis, not a finding: their agent is scoring a benchmark, where a worse trajectory is cheap, while an adopter's session is doing work where being right now outranks improving the procedure.
