...
The skill body was cut to about half its size to lower the per-run token cost. Each passage below is the original text, verbatim, of a paragraph or bullet that was removed or condensed; the skill now carries a shorter instruction in its place, or none. Fenced code blocks were not changed and are not repeated here.
...
**Run the shipped runner; do not write one on the spot.** Hand-written runners have reported a clean pass while checking nothing, in six separate ways. `scripts/verify-runner.sh` in this framework's clone handles all six. Run it **from the repo root**, with **absolute paths**, and include the project file (`CLAUDE.md`, `AGENTS.md`, or whatever your tool's naming map calls it):

Each probe runs with `bash -c` in the current directory, so a relative path in a probe depends on where you stand. Pass every file that holds a probe: a probe in a file you skip is never checked.

**Zero commands extracted is a defect, never a pass** — and so is a count the reader cannot account for. The `ran N of M` line reconciles commands run against `<!--`-shaped annotations in the same files; account for the difference item by item. Documentation of the syntax — code spans, fenced examples — is the expected explanation; an annotation the extractor could not see is a bug in the annotation or in the runner. The exit status says which case you are in: **2** means the run itself cannot be trusted — no files given, an operand that is not a readable file, nothing extracted, or nothing that produced a verdict because every annotation was manual or unreachable — **1** means a claim failed, errored or was malformed, and **0** means everything reachable checked out. Do not report a run you did not read the exit status of. **Count the rows you received against that line too**: only the reader can see a capture that lost rows — 77 of 109, once, under tmpfs pressure (#152).

**Dispositions** — first match wins, and the order matters because one command can satisfy several:

The four disposition-table rows whose cells were shortened, as they read before:
