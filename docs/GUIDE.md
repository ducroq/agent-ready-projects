...
**Version 1.45.0** | [Back to README](../README.md) | [Changelog](../CHANGELOG.md)
...
| **Start of session, after a framework you pin ships a release** | Agent finds the project's version stamps, lists the intervening releases and triages each into adopt / decline-with-reason / not-applicable / already-in-force — then stops before editing anything normative | Version stamps + the framework's changelog (a record of what was declined, and why) |
...
**Pre-commit review.** Install `review-changes` **user-globally** as `~/.claude/skills/review-changes/SKILL.md` — again from this repo's ready-made `.claude/skills/review-changes/SKILL.md` rather than from [`templates/review-changes.md`](../templates/review-changes.md) — and give each repo its own [`review-profile.md`](../templates/review-profile.md) at `<repo>/.claude/review-profile.md` — **and make sure that path is actually trackable**, since a `.claude/*` ignore with no negation leaves the profile untracked, `git status` clean, and every fresh clone refusing for want of a file the author can see (#166) — for a `/review-changes` skill that reads the pending diff and picks its review depth from what changed — a changelog edit gets one adversarial pass, a substantial change to a normative surface gets the full lens battery, and a small one gets a single independent pass unless it trips a carve-out. ⚠️ **This was the opposite instruction until v1.40.0**, and the reason it flipped is the v1.40.0 split: the risk tiers used to live in the skill, so one global copy would have imposed one tree's paths on every repo. They now live in the per-repo profile, so the skill is generic and belongs global — while the profile never travels. A project-local skill copy is now *inert*, shadowed by the global one. It complements deterministic checks (linters, structural tests) with the judgment-based kind: whether a change is broader than its stated intent, whether a document now contradicts itself.

**Framework drift.** For Claude Code, install `update-drift` **user-globally** as `~/.claude/skills/update-drift/SKILL.md` — copy the ready-to-use file from this repo's `.claude/skills/update-drift/SKILL.md`, which already carries valid frontmatter, **not** [`templates/update-drift.md`](../templates/update-drift.md), whose frontmatter sits inside its `SAVE AS` comment and therefore never registers (see [Where a skill lives](#where-a-skill-lives-user-global-or-project-local), rule 1). For other tools, run the template as a start-of-session prompt. Either way you get an `/update-drift` skill that finds a project's version stamps (tolerating the stamp shapes that occur in the wild), lists the releases between what the project pins and the framework's latest, and triages each one into **adopt / decline-with-reason / not-applicable / already-in-force** — plus a fifth for a pin that never resolved, which is a finding in itself — verifying claims by running them rather than by reading, and stopping before it edits any normative surface. It promotes `adopt.md` §3 (*Update — "Am I behind?"*) from a copy-paste prompt into a skill, which is what earns it a slot: §3 is the one of that file's three prompts that fires repeatedly, while *assess* and *adopt* fire once per project. ⚠️ **This is the skill whose absence hides every other absence** — nothing else tells you that a framework you adopted has moved.

**Releases.** Install `release` **project-locally** as `<repo>/.claude/skills/release/SKILL.md` — from a clone's ready-made `agent-ready-projects/.claude/skills/release/SKILL.md`, for the same reason as the two above, not from [`templates/release.md`](../templates/release.md) — for a `/release` skill that classifies the semver bump, verifies preconditions, drafts the changelog entry, and syncs version strings across the repo — then stops before tagging. Note this one ships `disable-model-invocation: true`: it is user-invoked only, never something the agent starts on its own. The judgment worth automating here is the bump decision, which otherwise lives in a prose header and drifts.

**Why these five and not more.** Each attaches to a recurring moment with a real decision in it — end of session, monthly, pre-commit, per release of the framework you pin, per release of your own. The other rows in the rhythm table are single writes with no branching: logging a gotcha is a two-line append, and wrapping it in a skill adds ceremony without adding judgment. Skills are worth their slot when they encode a *procedure you'd otherwise get wrong*, not when they merely save typing. Their names and descriptions occupy context in every session, so a small, well-chosen set outperforms a large one — the auto-loading cliff applies to tooling, not just documents.
...
| `update-drift` | **global** | The drift question is identical in every repo that pins a framework. Its backticked paths are convention paths, `<framework>`/`<name>` parameters, or paths in the **framework's** tree — the skill's subject rather than its host; its one machine-layout reference (`~/repos/<framework>/`) is a hint it offers *if one exists*, never a path it requires. The genuinely repo-specific part — which directories to sweep for version stamps — is chosen at run time rather than baked in, so one copy serves every repo. |
...
| `review-changes` | **global since v1.40.0** | It was project-local for as long as the risk-tier table and guarantee lens named real files in the real tree — two adopters' copies were different artifacts sharing a name. Both now live in a per-repo `.claude/review-profile.md`, so the skill body is generic and one copy serves every repo. ⛔ It STOPS where no profile exists, rather than defaulting every path to LOW. |
| `review-profile.md` | project-local, one per repo | Not a skill — the per-repo half of `review-changes`. Never global: it *is* the repo-specific part. |
...
2. **A frontmatter pin is part of the installed bytes, so it is part of what the verifier compares.** Claude Code's skill frontmatter appears to accept an `effort:` key — in the 2.1.263 bundle the name sits in a key list beside `allowed-tools`, `disable-model-invocation` and `user-invocable`, and an effort enum of `low, medium, high, xhigh, max` occurs elsewhere in the binary. ⚠️ **That is an adjacency in a string table, not a schema this repo has exercised**, and the two facts were read from different places: treat the key as probable and the value list as unconfirmed until your own tool tells you otherwise. The list also mixes in plugin-manifest keys, so it is a key table rather than *the* frontmatter schema. The consequence for scope is the part that is checkable: `install-global-skills.sh --check` compares the install against **the tracked copy in your working tree**, so a pin added only to the global copy reports as drift until you put the same pin in the tracked source and reinstall. Releasing does not clear it. (The release tag is consulted, but only to *excuse* an install that still matches the release while the tree has moved ahead of it — so on an unreleased tree a pin in the tracked source reads as `nothing to refresh yet`, and it is the release that turns it into drift.) (The release tag governs the *install* path, which refuses to copy unreleased bytes; that is rule 1, and it is a different comparison.) Pin in the tracked source and ship it, or do not pin. **This is a Claude Code mechanism.** Whether the other tools in the naming map accept an equivalent key has not been measured here — assume nothing about them either way.
3. **Never leave an inert local copy behind.** When a skill moves to global scope, delete the project-local copies in the same change. A shadowed copy will drift from the one actually in use, and the drift is invisible precisely because the drifted file is never loaded.
...
| "Drift command" | `~/.claude/skills/update-drift/SKILL.md` (`/update-drift`, user-global) | Start-of-session prompt | Start-of-session prompt | Start-of-session prompt | Start-of-session prompt | Start-of-session prompt |
| "Review command" | `~/.claude/skills/review-changes/SKILL.md` (`/review-changes`, user-global) + a per-repo `.claude/review-profile.md` | Pre-commit prompt | Pre-commit prompt | Pre-commit prompt | Pre-commit prompt | Pre-commit prompt |
| "Release command" | `<repo>/.claude/skills/release/SKILL.md` (`/release`, project-local) | Release-time prompt | Release-time prompt | Release-time prompt | Release-time prompt | Release-time prompt |
