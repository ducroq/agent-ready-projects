...
- **Self-verifying memory** (v1.9.0) — agents embed `<!-- verify: command -->` in state claims on write, execute them on read, and audit them on curate. No other framework distinguishes state claims from observations in memory, or provides a mechanism for memory entries to prove themselves true. Motivated by the news-aggregator incident ([issue #8](https://github.com/ducroq/agent-ready-projects/issues/8), 230 articles affected by a false "shipped" claim); validated by the ETH Zurich finding that static context files can hurt performance.
