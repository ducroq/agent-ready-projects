...
- **Any non-frontmatter edit under `.claude/skills/**`** — HIGH because a defect there ships to every install derived from it; that is as true of a three-line body edit as of a frontmatter one. *(That bullet is written to survive a formatter, and needs to be. A `**`-suffixed glob inside a bolded phrase is corrupted by prettier: the closing `**` becomes a literal `\*\*` and the phrase loses its bold, or with two globs the spaces between them are eaten. **Measured**: breaks on prettier 2 and 3.8.1, fixed in 3.9.6 — so the boundary is inside the 3 series, not between 2 and 3. What protects a line is a **later code span on it**; that is why this bullet, with its parenthetical, survives while a bare one does not. Do not rely on that: put extra paths in a parenthetical rather than a second glob, and check a rendered view. Step 1.5 does not catch this — it checks tables and fences, not emphasis.)*
