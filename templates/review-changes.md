...
- **Frontmatter edits to those same files** — removing one `---` silently unregisters a skill.
  ...
  *Both bullets used to end a bolded phrase with a `**`-suffixed glob, and prettier corrupts that shape. **The rule worth remembering is the shape** — never end a bolded phrase with such a glob; put the path in a parenthetical, as above. Step 1.5 catches this exact shape. The prettier versions, the masking rule and why this shipped broken for a day live in `docs/rationale/review-changes.md` — nobody needs them to apply the rule.*
