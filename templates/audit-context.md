...
| **rung 4**, a sibling named in prose | adjudicates — mislabelled; the remedy is *qualify it instead* | Findings |
...
⚠️ **Row 5 is the one to get right**: resolving nowhere on a tree that *could* test it is the placeholder doing its job. A draft of this table said *Findings* there — reporting every legitimate placeholder as a defect — and the suite stayed green, because the oracle agreed with the prose and not with the table.

The angle-bracket row is the **fall-through** only: rungs 1 and 1b run first and can report such a path (carve-out below), so the shape decides where it lands once nothing local answered, not whether it is looked at. A path carrying both forms follows the shape; two drafts got that ordering wrong in the two available directions.
...
**Before marking a cross-repo path, try qualifying it instead** — name the repo in the surrounding prose so rung 4 resolves it. A qualified reference is checked on every run; a marked one is checked only for *mislabelling* — that it **does** resolve after all, at one of the adjudicating rungs in the table above — and never for decay. *(This read "never checked again" until #93. That was true before #73 added the stale-marker rungs, and the sentence outlived them by two releases while the step 38 lines below now states the opposite.)*
...
⚠️ **This does not make the status a property of the repo, and nothing can.** Rung 4's gate is a repo *name in the prose*, and a name is only recognisable as a repo name when that repo is on disk — so whether an individual reference was decidable is not computable, and *was any neighbour reachable* is a coarse stand-in for it in both directions. One unrelated clone next door is enough to make rung 4 count as having run for a reference naming a repo that is absent, and that reference is then reported as a defect. What the third state buys is narrower and still worth having: **an undecided run says so, instead of picking one of the two verdicts it has not earned.** ⚠️ **There is one move, not two, and it is scoped.** Run the audit where the neighbours are reachable. *Marking* is not a second move **for a reference no local rung resolves**: it is rung-4 traffic either way, so it lands in the same undecided section at the same status. Until v1.30.0 this paragraph said that of *every* marked reference and of the run's "ability to decide it" — unhedged, and false for anything a local rung answers.

⚠️ **A `<!-- placeholder -->` marker is rung-4 traffic too — and an angle-bracket segment is not.** The stale-marker test runs a marked path against the neighbours, so where none is reachable that test cannot run and such a reference is *undecided*, not excused; excusing it silently returns *clean* on a repo where a reachable neighbour would have reported a stale marker. — but only once nothing local has answered it, which is the order the table above gives. **An angle-bracket segment is decided by its shape instead**, so rung 4 declines it on every tree measured and it stays excused; treating it as undecided moves a repo whose placeholders are all of that form from *clean* to *coverage incomplete* on a fresh clone, which is this step's own defect one bucket over. ⚠️ **"Shape" does not mean nothing on disk can touch it.** A literal file of that name — `docs/<slug>.md` on disk, next to the document naming it — resolves at rung 1 or 1b and is reported, with a wording that does **not** say *stale marker*, since there is no marker to remove. A *neighbour* holding such a file suffix-matches at rung 4 the same way. Both are rare (the name is illegal on NTFS), both measured.
