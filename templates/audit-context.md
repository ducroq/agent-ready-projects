...
- **Layer 3 present**: check that every topic file has a task-triggered pointer in the "Before You Start" table. Flag orphaned topic files — they exist but no pointer leads to them, so an agent will never know to load them. Also check that every work-item file in `docs/work-items/` (other than `README.md`) has a pointer in the index's Current State section, and that no pointer names a file that no longer exists.
- **No Layer 3 anywhere** — the case for every tool without auto-memory, where `docs/GUIDE.md` says plainly that everything goes into the project file: report this step as **not applicable, and say why**. A skipped check and a passing check are indistinguishable in a report that says neither, which is the whole reason this bullet exists rather than the step quietly doing nothing.

**Do not substitute `docs/*.md` for the topic files, and do not invent a home for the work-item pointers.** `docs/` is where this framework puts essays and reference material — twelve files in this repo alone — so treating it as a topic-file directory reports every essay as an orphan. That is the failure class v1.15.1 spent a release removing from Step 4. The work-item half is the subtler trap: `docs/work-items/` is tool-independent, so it is tempting to check it anyway against the project file — but `templates/work-item.md`, `templates/README.md` and `docs/GUIDE.md` all say the pointer goes in *the memory index's Current State section*, unconditionally, and `templates/project-file.md` ships no such section. An adopter without Layer 3 who followed every template therefore has work-item files and, by construction, nowhere the pointer could have gone. Checking it would flag all of them. **Report that as a gap in the framework, not as a finding about the project** — it is tracked as issue #44, and where those pointers should live is undecided.
