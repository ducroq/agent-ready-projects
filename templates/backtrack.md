...
<!-- SAVE AS: ~/.claude/skills/backtrack/SKILL.md (Claude Code, USER-GLOBAL — see docs/GUIDE.md
     "Where a skill lives"; do not copy this file verbatim, its frontmatter is
     inside this comment. Prefer .claude/skills/backtrack/SKILL.md from this repo.)
     For other tools, run this as a mid-session prompt manually.

     This is a skill (/backtrack) that reorients a session that has drifted
     from its opening ask. It tends the SESSION's thread; curate tends the
     project's memory layers. Backtrack, then curate.
