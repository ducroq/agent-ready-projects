...
framework: agent-ready-projects v1.46.0   # a NUMBER, not a status — never write "current" here; the framework's release cadence falsifies the adjective, not the pin
---
...
- **Lead with the point; no narrative.** Each entry is a fact or a rule in a line or two. How it got that way goes to memory or the changelog, not here: this file is loaded every session, and narrative is what grows it.
- [ constraint 1 ]
- [ constraint 2 ]
